"""
================================================================
🐂 BOGA AI SWING TRADE MODEL — V115.0 (Transparency + Edge)
================================================================
V115 YENİLİKLER:
...
RSI_MIN_SWING = 42  # 🔧 FIX: 35 → 42. Aşırı zayıf hisseler (downtrend) elensin.
RSI_MAX_SWING = 75  # 🎯 SNIPER MOD: 72 → 75. Momentum breakout'larda 75'e kadar tolerans.
...
# YENİ:
SLOW_PEAK_SECTORS = {
    "Real Estate": -5.0,  # 🎯 REITS 7 günlük hıza asla yetişemez, ağır ceza.
    "Energy": -3.0,
...
def calculate_support_resistance_1h(df_1h: pd.DataFrame, df_1d: pd.DataFrame, current_price: float, entry_trigger_1d: str = "", df_15m: pd.DataFrame = None) -> dict:
    ...
    try:
        ...
        is_momentum_entry = entry_trigger_1d and any(k in entry_trigger_1d for k in ["Squeeze", "Awakening", "Momentum", "Spring"])
        rr_cap = 6.0 if is_momentum_entry else 4.0
...
async def analyze_market_and_sectors():
    """
    V115: Market regime (VIX + SPY) ve sektör performans analizi.
    
    DEĞİŞİKLİKLER (V114 → V115):
      - VIX zaten çekiliyordu ama sadece eşik kontrol ediyordu.
      - V115: VIX slope (yükseliyor mu?) + VIX percentile + override logic eklendi.
    ...
    # ── STEP 5: SEKTÖR PERFORMANSI (V114 ile aynı, değişmedi) ────────
    for sector_name, etf_ticker in SECTOR_ETF_MAP.items():
        try:
            ...
            if len(hist_21d) >= 5:
                ...
                # 5 günlük ağırlık %60, 21 günlük %40 → V114'teki mantık korundu
...
def calculate_profit_target(entry_price, atr_value, momentum_score, is_exhausted=False, beta=1.0):
    """V115 — ATR Based Dynamic TP/SL (Realistic swing band: %8-15, mostly hit within 5-10 days)
...
async def apply_atmaca_filters(ticker: str) -> Optional[dict]:
    ...
    try:
        ...
        # 🎯 0-DAY SNIPER HARD REJECTS
        # 🚨 FIX: Sektöre özel (Utilities red / Real Estate istisnası) kurallar tamamen silindi.
        # Algoritma isimlere/sektörlere değil, saf hıza (Beta matematiğine) odaklanacak şekilde hizalandı.
        if beta > 0 and beta < ATMACA_MIN_BETA:
            return None
        ...
        # 🚨 FIX: Sadece açılışın üzerinde kapatması yetmez. Gerçek bir alıcı baskısı (Hammer/Pinbar) görmek için
        # mumun o günkü toplam marjının (High-Low) üst %50'sinde kapanmasını şart koşuyoruz.
        daily_range = high_1d.iloc[-1] - current_low
        is_spring = (current_low < min_low_10d) and (current_price > min_low_10d) and (current_price > df_1d['Open'].iloc[-1]) and (current_price >= current_low + (daily_range * 0.5))
        ...
        # V115: ADX slope-bazlı puanlama (seviye değil, ivme önemli)
        try:
            adx_slope = adx_series_1d.diff().tail(5).mean()
        except Exception:
            adx_slope = 0.0

        # Level puanı: Trend varlığı onayı (konservatif)
        if adx_1d >= 40:
            score -= 3.0; details.append(f"⚠️ ADX: Peak Zone / Trend Exhaustion ({adx_1d:.1f})")
        elif adx_1d >= 28:
            score += 2.0; details.append(f"💪 ADX: Established Trend ({adx_1d:.1f})")
        elif adx_1d >= 18:
            score += 5.0; details.append(f"🔥 ADX: Trend Awakening — OPTIMAL ENTRY ZONE ({adx_1d:.1f})")
        else:
            score -= 2.0; details.append(f"🟡 ADX: Very Weak ({adx_1d:.1f})")

        # 🚨 FIX 5: ADX Double Counting düzeltildi (slope puanları düşürüldü).
        if adx_slope > 0.8:
            score += 2.0; details.append(f"⚡ ADX Slope: Aggressive (+{adx_slope:.2f}/bar)")
        elif adx_slope > 0.3:
            score += 1.0; details.append(f"📈 ADX Slope: Rising ({adx_slope:.2f}/bar)")
        elif adx_slope < -0.5:
            score -= 3.0; details.append(f"🐌 ADX Slope: Fading ({adx_slope:.2f}/bar)")
        ...
        # 1. VOLUME TIGHTENED: Hacim Kuruması (Dry-up) Filtreleri
        min_rvol_required = 0.80 if sector_name == "Real Estate" else 1.05
        
        # 🚨 FIX 3: Pre-breakout ignition (Dirence çok yakın sıkışma) durumunda hacimsizliği tolere et.
        try:
            macro_resist = float(high_1d.tail(15).max())
            breakout_distance = (macro_resist - current_price) / current_price
            # Fiyat dirence %2.5'ten daha yakınsa, kopuş öncesi hacim sessizliği çok sağlıklıdır. Eşiği esnet.
            if 0 <= breakout_distance < 0.025: 
                min_rvol_required = min(min_rvol_required, 0.85)
        except Exception: 
            pass

        # 🔧 FİNAL KONTROL: Squeeze, Spring veya dirence yakın formasyonlarda hacim daralmasına izin ver.
        ...
        try:
            ...
            # 🎯 SNIPER MOD: EMA200 yeterli. EMA50 altındaki base-building (sıkışma) hisseler dahil.
            # 🚨 FIX: EMA200 altındaki Squeeze (Daralma) bir Ayı Bayrağıdır (Bear Flag). Sadece SPRING'e izin ver.
            if current_price < ema200_val and not is_spring:
                layer2_pass = False; layer2_reasons.append("Below EMA200 without spring")
            
            if len(close_1d) >= 6:
                ret_5d_pct = (current_price - float(close_1d.iloc[-6])) / float(close_1d.iloc[-6]) * 100
                # 🎯 SNIPER MOD: 5D return koşulu -5%'e çekildi.
                # Pullback içinde sıkışan hisseler en iyi patlama adaylarıdır.
                # 🚨 FIX: -5.0 → -3.5. 5 günde -%5 düşen hisse pullback değil, kan kaybediyor.
                if ret_5d_pct < -3.5:
                    layer2_pass = False
                    layer2_reasons.append(f"5-day return {ret_5d_pct:.2f}% (bleeding/downtrend)")
                # (Yatay hisseler artık reddedilmiyor çünkü yataylık = Squeeze demektir)
        except Exception:
            pass
        ...
        # 🔧 FIX: Squeeze/Spring sırasında kırmızı mumlar konsolidasyon belirtisidir — 3'e indir.
        min_green_required = 4 if sector_name == "Real Estate" else 3 if (is_squeeze or is_spring) else 5
        ...
        # 4. MONEY FLOW DYNAMIC GATE
        # 🔧 FIX: 0.0 → -0.05. Squeeze aşamasında CMF geçici negatife düşer (dry-up normaldir).
        # Gerçek dağıtım sinyali -0.05 altındadır.
        ...
        elif cmf_val < 0.0 and not (is_squeeze or is_spring):
        ...
        try:
            rsi_series_quick = RSIIndicator(close_1d, 14).rsi()
            rsi_quick = float(rsi_series_quick.iloc[-1])
            rsi_prev = float(rsi_series_quick.iloc[-2])
            if rsi_quick < RSI_MIN_SWING:
                layer2_pass = False; layer2_reasons.append(f"RSI={rsi_quick:.1f}<{RSI_MIN_SWING} (weak)")
            # 🚨 FIX: HOG (Düşen Bıçak) Koruması. RSI 45'in altındaysa VE yönü hala aşağı bakıyorsa dibi tahmin etmeye çalışma.
            elif rsi_quick < 45 and rsi_quick < rsi_prev:
                layer2_pass = False; layer2_reasons.append(f"RSI={rsi_quick:.1f} DOWNWARD (Falling Knife)")
        except Exception:
            pass

        # 6. TREND STATUS GATE: explicit downtrend reject.
        # 🚨 FIX: is_spring (Tuzak/Dip Dönüşü) durumunda downtrend olması normaldir, istisna sağlandı.
        if trend_durumu_1d == "Downtrend" and not is_spring:
            layer2_pass = False; layer2_reasons.append("Trend Status=Downtrend (No Spring)")

        if not layer2_pass:
            return None

        # ============================================================
        # [START] SMART NETWORK TRANSITION — ONLY PASSING LAYER 2 ~50 STOCKS
        ...
        # 🚀 FIX: 15m analizini önceki yamalarda iptal etmiştik. Boşuna veri çekip 
        # API'yi yormamak ve botu hızlandırmak için fetch işlemi kapatıldı.
        df_15m = None
        ...
        if atr_pct_1d < 0.025:
            ...
            score += 6.0; details.append("⚡ VOL Regime: Good Swing")
        elif 0.045 <= atr_pct_1d <= 0.075:
            score += 8.0; details.append("🔥 VOL Regime: 7g HEDEF Sweet Spot")
        elif 0.075 < atr_pct_1d <= 0.100:
            score += 4.5; details.append("🟧 VOL Regime: High (managed risk)")
        else:
            score += 1.0; details.append("🟨 VOL Regime: Extreme (risky)")
        ...
        if sector_name == "Real Estate":
            ...
            # 🚨 FIX 6: Explosive momentum liderleri için RSI tavanı göreceli güce (RS) bağlı esnetildi.
            ...
            elif 68 < rsi_1d_val <= 78 and is_strong_rs:
                score += 2.0; details.append(f"🚀 RSI: Strong Momentum Leader ({rsi_1d_val:.1f})")
            ...
            elif rsi_1d_val > 78 or (rsi_1d_val > 75 and not is_strong_rs):
        ...
        try:
            if len(rsi_1d_series) >= 5:
                ...
                if rsi_slope_5 < -5:
                    score -= 6.0; details.append(f"📉 RSI Slope: Güçlü düşüş ({rsi_slope_5:.1f})")
                elif rsi_slope_5 < -2:
                    score -= 3.0; details.append(f"↘️ RSI Slope: Hafif düşüş ({rsi_slope_5:.1f})")
                elif rsi_slope_5 > 5:
                    score += 4.0; details.append(f"📈 RSI Slope: Güçlü yükseliş ({rsi_slope_5:.1f})")
                elif rsi_slope_5 > 2:
                    score += 2.0; details.append(f"↗️ RSI Slope: Yükselişte ({rsi_slope_5:.1f})")
        except Exception:
            rsi_slope_5 = 0.0
        ...
        try:
            ...
            # 🚨 FIX 1: Exhaustion Paradox! 3 günde %5 artan bir hisse yorgun değil, taze bir momentumdur. 
            # RSI sadece aşırı şiştiğinde (78+) veya 3 günde parabolik uçtuğunda (%12+) Exhausted sayılır.
            ...
            if roc_3d > 12.0 or is_overbought_rsi or ret_1d_pct > 6.0:
                is_exhausted = True
                reasons_ex = []
                if roc_3d > 12.0: reasons_ex.append(f"3G ROC: +{roc_3d:.1f}% (Parabolic Move)")
                if ret_1d_pct > 6.0: reasons_ex.append(f"1G ROC: +{ret_1d_pct:.1f}% (1D Spike FOMO)")
                if is_overbought_rsi: reasons_ex.append(f"RSI: {rsi_1d_val:.1f} (Overbought Peak)")
        ...
        try:
            ...
            if macd_hist_val > 0 and macd_hist_val > macd_hist_prev:
                ...
                score += 1.5; details.append(f"[OK] MACD Hist: Positive ({macd_hist_val:.3f})")
        ...
        rsi_1h_slope = 0.0  # 🔧 FIX: Dual-TF block için başlangıç değeri
        ...
        if df_1h is not None and len(df_1h) >= 10:
            ...
            rvol_durumu = "🔥 EXTREMELY INTENSE" if rvol_1h > 3.0 else "[OK] High" if rvol_1h > 1.5 else "❄️ Volumeless" if rvol_1h < 0.7 else "Normal"
            ...
            # YENİ:
            # 🎯 0-DAY SNIPER: Fiyat EMA20'den %3 uzaklaştıysa FOMO'dur. Puanı tamamen çökert!
            if ema20_distance > 0.03:
                score -= 20.0; details.append(f"🔴 1H: EXTREME FOMO RISK (+{ema20_distance*100:.1f}% from EMA20)")
            elif ema20_distance > 0.015:
                score -= 6.0; details.append(f"🟡 1H: Extension Risk (+{ema20_distance*100:.1f}%)")
            ...
            try:
                ...
                if len(rsi_1h_series) >= 3:
                    ...
                    if rsi_1h_slope < -8:
                        score -= 5.0; details.append(f"📉 1H RSI Slope: Sert düşüş ({rsi_1h_slope:.1f})")
                    elif rsi_1h_slope < -3:
                        score -= 2.5; details.append(f"↘️ 1H RSI Slope: Düşüşte ({rsi_1h_slope:.1f})")
                    elif rsi_1h_slope > 8:
                        score += 3.0; details.append(f"📈 1H RSI Slope: Sert yükseliş ({rsi_1h_slope:.1f})")
                    elif rsi_1h_slope > 3:
                        score += 1.5; details.append(f"↗️ 1H RSI Slope: Yükselişte ({rsi_1h_slope:.1f})")
            except Exception:
                rsi_1h_slope = 0.0

            # ── HER İKİ TF'DE RSI DÜŞÜŞ — HARD BLOCK ────────────────
            try:
                if rsi_slope_5 < -4 and rsi_1h_slope < -4:
                    score -= 15.0; details.append(f"🚫 DUAL-TF RSI DÜŞÜŞ: 1D({rsi_slope_5:.1f}) + 1H({rsi_1h_slope:.1f}) — Her iki TF'de momentum kayıpda!")
            except Exception:
                pass
        ...
        # ── MTF LOCK ───────────────────────────────────────────────
        # 🚨 FIX: Ayı bayraklarından (Downtrend Squeeze) korunmak için squeeze muafiyeti kaldırıldı.
        ...
        if obv_slope > 1000: score += 6.0; details.append("[OK] OBV: Strong Accumulation")
        ...
        # 🔧 BONUS FIX: Restored proper churn-detection logic.
        # 🚨 FIX: Eğer bu bir SPRING (Dip Dönüşü/Çekiç Mumu) ise, yüksek hacimli küçük gövde
        # mal dağıtımı değil, güçlü bir alım/likidite temizliğidir. İstisna sağlandı.
        ...
        # 🔧 FIX: %5+ spike cezası yukarıda Exhaustion bloğunda (-20p) zaten verildi.
        # Çifte ceza (double-penalty) kaldırıldı. Sadece %8 üstü kesin red kaldı.
        ...
        try:
            ...
            # 🚨 FIX 2: Squeeze vs Dead Money Çelişkisi çözüldü. Sıkışan bir hissenin 20 günlük marjı
            # doğal olarak çok dar olacaktır. Squeeze etiketli hisselerin ölü sanılıp silinmesi engellendi.
            if 0 < price_20d_range < 0.05 and not is_squeeze:
                return None  # Sadece Squeeze DIŞINDAKİ gerçekten ölü hisseleri ele
        except Exception:
            pass
        ...
        if 1.2 <= rvol_today <= 1.8 and abs(close_change_pct) < 0.006:
            ...
            score += 8.0; details.append(f"[START] RVOL Regime: Swing Awakening ({rvol_today:.2f}x)")
        elif rvol_today > 1.5:
        ...
        if market_regime == "STRONG":
            ...
            if vol_5d_trend >= 0.80: score += 1.6; details.append("[OK] Bullish Market Alignment")
        ...
        entry_trigger = None
        selection_system = "MOMENTUM"          # varsayılan
        selection_reasons: List[str] = []      # çoklu sinyal kaynağı
        ...
        # Stealth Breakout Definition
        is_early_awakening = (current_price > ema20_now) and (45 <= rsi_1d_val <= 55) and (0.9 <= rvol_today <= 1.3)

        # Spring/Failed Breakdown zaten is_spring olarak hesaplanmış (yukarıda)
        if is_spring:
            selection_system = "SPRING"
            selection_reasons.append("Failed_Breakdown")

        # 🚨 FIX: "and not is_squeeze" kaldırıldı — bb_squeeze=True iken is_squeeze da True olur, koşul asla çalışmazdı.
        if bb_squeeze and ema_stack and rvol_today > 1.2:
            score += 25.0; entry_trigger = "BB Squeeze + EMA Stack + Volume"
            selection_system = "SQUEEZE"
            selection_reasons.append("BB_Squeeze")
            selection_reasons.append("EMA_Stack")
            selection_reasons.append("Volume_Confirm")
            details.append("💥 ENTRY: DAY 0 SQUEEZE BREAKOUT (SNIPER)")
        elif is_early_awakening:
            score += 18.0; entry_trigger = "Early Awakening (Stealth Breakout)"
            selection_system = "AWAKENING"
            selection_reasons.append("Stealth_Breakout")
            selection_reasons.append("RSI_Sweet_Spot")
            details.append("🌅 ENTRY: DAY 0 EARLY AWAKENING (Before the Crowd)")
        elif ema_cross and 1.1 <= rvol_today <= 1.8:
            score += 10.0; entry_trigger = "EMA9/20 Crossover + Micro Volume"
            selection_system = "EMA_CROSS"
            selection_reasons.append("EMA9_20_Cross")
            selection_reasons.append("Micro_Volume")
            details.append("🎯 ENTRY: EMA9/20 Fresh Cross")
        # 🚨 FIX: "and not is_squeeze" kaldırıldı.
        elif ema9_slope > 0.003 and bb_squeeze and micro_volume:
            score += 15.0; entry_trigger = "EMA9 Slope + Squeeze + Micro Volume"
            selection_system = "SQUEEZE"
            selection_reasons.append("EMA9_Slope")
            selection_reasons.append("BB_Squeeze")
            selection_reasons.append("Micro_Volume")
            details.append("⚡ ENTRY: EMA9 Dynamic Squeeze Break")
        elif ema20_now > ema50_now_1d and close_change_pct > 0.006:
            score -= 2.0; entry_trigger = "Trend Continuation (Late)"
            selection_system = "BREAKOUT"
            selection_reasons.append("Trend_Continuation")
            details.append("⚠️ ENTRY: Trend Continuation (FOMO Risk, Late Phase)")
        elif rising.get('is_rising') and rising.get('pattern') in ['Pullback Reversal', 'Base Breakout']:
            score += 6.0; entry_trigger = f"Rising: {rising['pattern']}"
        ...
        # 🔧 FIX: Mantık DÜZELTİLDİ. entry_trigger OLAN hisse zaten iyi sinyal vermiş →
        # daha GEVŞEK eşik (1.5) uygula. entry_trigger YOK ise daha SIKI (1.8) ol.
        if rr_ratio_calc < (MIN_RR_RATIO if entry_trigger else MIN_RR_RATIO_RELAXED):
        ...
        # 🎯 FIX 6: Yavaş sektörlere ceza (DİNAMİK)
        sector_penalty = SLOW_PEAK_SECTORS.get(sector_name, 0.0)
        sec_perf_val = SECTOR_PERFORMANCE.get(sector_name, 0.0)
        
        if sector_penalty < 0:
            if sec_perf_val > 2.0:
                details.append(f"🔥 Slow Sector But HOT Trend: {sector_name} (Ceza İptal)")
            else:
                score += sector_penalty
                details.append(f"🐢 Slow Peak Sector: {sector_name} ({sector_penalty:.1f}p)")
            
        exhaust_tag = " [EXHAUSTED]" if is_exhausted else ""
        logging.info(f"[OK] {ticker}: Analysis complete (Score: {score:.2f}{exhaust_tag})")
...
def compute_boga_score_100(c: dict) -> float:
    ...
    # ADX (0-10 puan) — V115: Slope-aware, apply_atmaca_filters ile tutarlı
    # Mantık: ADX 18-28 arası YÜKSELEN = prime entry (trend başlangıcı)
    #          ADX 28-40 arası = established trend (geç ama geçerli)
    #          ADX 40+   = exhaustion zone (trend yorgunluğu, ceza)
    adx = c.get("adx", 0.0)
    adx_slope = c.get("adx_slope_val", None)  # apply_atmaca_filters'tan gelirse kullan

    if adx >= 40:
        score_100 += 0.0   # Trend exhaustion — puan yok
    elif 28 <= adx < 40:
        score_100 += 6.0   # Established trend — makul ama geç
    elif 18 <= adx < 28:
        score_100 += 10.0  # OPTIMAL ZONE — trend awakening, prime entry
    elif adx >= 15:
        score_100 += 3.0   # Zayıf ama var
    # adx < 15: 0 puan (apply_atmaca_filters zaten eleyor çoğunu)
...
# ================================================================
# ================================================================
# SECTION 15: TELEGRAM REPORT BLOCK — BOGA AI Core Engine v115
# ================================================================
# ================================================================
...
def build_candidate_block(rank: int, c: dict) -> str:
    """
    V115: Sistem etiketi ve selection_reasons eklendi.
    """
    ticker      = c.get("ticker", "")
...
def track_pick_peak_performance():
    """
    V115: Geçmiş seçimlerin peak % performansını otomatik hesaplar.
...
async def scan_top_stocks():
    ...
    # top_candidates da boga_score_100'e göre sırala (Telegram ile terminal tutarlı olsun)
    top_candidates.sort(key=lambda x: x.get("boga_score_100", 0.0), reverse=True)
    
    # ── V115: Terminal'de 20 hissenin tam listesi (boga_score_100 artık hazır) ──
    logging.info("=" * 70)
    logging.info(f"🐂 BOGA AI V115 — TAM ADAY LİSTESİ ({len(top_20_candidates)} hisse)")
    ...
    header = (
        f"🐂 <b>ATMACA SWING V115 – TOP 5 PREMIUM PICKS</b>\n"
    ...
    for i, c in enumerate(tg_display_picks):
        ...
        # V115: Sistem kısaltması (7 karakter max, tablo bozulmasın)
    ...
    toplist_msg = header + "\n".join(rows) + "\n─────────────────────────────────────────────────────\n</pre>\n"
    toplist_msg += f"<i>[INFO] BUY→SELL: R/R~2.5:1 | ATR+1H Support/Resistance | BOGA AI V115</i>\n\n"
    ...
    # V115: Peak performance tracker (sistem bazlı winrate birikimi)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if "--oneshot" in sys.argv:
            ...
            print("[START] BOGA AI v115.0 Swing Scanner (One-Shot) baslatildi...")
            asyncio.run(scan_top_stocks())
            print("[OK] Tarama tamamlandi.")
        else:
            asyncio.run(run_scanner())
    except KeyboardInterrupt:
        print("\n🐂 BOGA AI v115.0 durduruldu.")
    except Exception as e:
