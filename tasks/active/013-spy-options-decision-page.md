...
- **Phase 3 — done**: `frontend/lib/spyengine/heuristics.ts` — BVC (Bulk Volume Classification) volume split, `computeExhaustion()` (opposing-side volume fade, LONG/SHORT), `computeReversalScore()` (Layer 1 RSI/MACD + RVOL + exhaustion, explicitly documented as uncalibrated heuristic weights, never labeled "probability"). No VIX/SPX inputs (per §2.6/§2.7).
- **Phase 5 — done**: dedicated "SPY Option" tab added to `/admin/spyengine/v1` (right after "Kumanda Merkezi", per user request) combining all five tiers: Tier 1 (Monte Carlo, now run across 6 horizons — 5/10/.../30 min — for a fiyat×zaman matrix), Tier 2 (delta-implied market probability from the Phase-2 option snapshot), Tier 3 (Reversal Score/Exhaustion cards, LONG+SHORT), Tier 4 (reuses the Phase-2 Greeks/premium data, not recomputed), Tier 5 (continuous 0-100 Edge Score bars, no hard-gate). Chart uses the project's own SpyChart component (`levelLines` prop) to plot Tier 1/5 levels directly on the 5m candles — no new charting library. `frontend/lib/spyengine/heuristics.ts` + a `decisionPage` field on the v2 route computes everything server-side, deterministic per closed 5m bar (same `seedFromBar` mechanism as Phase 1), so the page updates exactly on the 5-minute cadence the user asked for.
- **Phase 4 prep — done**: `frontend/lib/spyengine/decisionLogStore.ts` + `appendDecisionSnapshot()` wired into the v2 route logs one snapshot per newly-closed 5m bar (deduped by `barTime`, both in-memory per server instance and idempotently in the stored payload) to Supabase `shared_store` key `spyengine_option_decision_log` — Tier 1 (30-min touch probabilities), Tier 2 (nearest-strike implied prob), Tier 3 (raw scores), Tier 5 (ATM edge). Fire-and-forget: a write failure never affects the live response. Documented in `docs/DATA_CONTRACTS.md`. **This only accumulates inputs+predictions** — the actual Phase 4 work (a "grading" pass matching snapshots against realized outcomes, then logistic regression) still needs enough days of history before it can start, and hasn't been written yet.
...
- [x] Market-implied anchor comes only from real live option-chain data; `null` if the chain fetch fails — Tier 2 derived directly from `optionDecision` deltas, no fallback fabrication
...
- [x] No placeholder values (`-0.XX`, `+$XXX`) ship to production — every number on the SPY Option tab traces to a real computation or is `null`/"veri yok"
- [x] Reversal Score / Seller Exhaustion are labeled "Skor", not probability, until calibrated — enforced in both `heuristics.ts` naming and the tab's UI copy/warning banner
- [x] Every number on the new tab is visually tagged with its Tier (1-5) — each panel titled "Tier N — ..."
- [x] Edge Skoru is continuous (0-100); no hard-gated "NO TRADE" zone — rendered as a proportional bar per level, never hidden/blocked
- [x] Decision-page snapshots are logged from first ship for future calibration — `decisionLogStore.ts`, one entry per closed 5m bar
- [x] New tab sits after "Kumanda Merkezi" in `/admin/spyengine/v1` — titled "SPY Option"
