...
async def apply_atmaca_filters(ticker: str) -> Optional[dict]:
    """
    BOGA AI V117.2 — 1W → 1D → 1H Hiyerarşik Filtre Mimarisi
    ...
    try:
        ...
        try:
            mfi_series = MoneyFlowIndexIndicator(high_1d, low_1d, close_1d, volume_1d, 14).money_flow_index()
            mfi_val = float(mfi_series.iloc[-1])
        except Exception:
            mfi_val = 50.0

        if mfi_val > 75 and rsi_quick > 65:
            score -= 5.0  # raw_score'daki erken uyarı cezası (çifte sayımı engellemek adına -5p)
...
# ── 1D HARD GATE'LER — V117_v2 ────────────────────────────────
        ...
        # ── V117: AWAKENING — Sıkılaştırılmış 7 Koşul ────────────────
        # Önceki (V117): sadece 3 koşul → günde 50+ hisse geçiyordu
        # Yeni (V117): 7 koşul → sadece gerçek stealth breakout adayları
        #
        # Koşul 1-3: V117'den korunan temel koşullar
        # Koşul 4: 1W RSI ≥ 50 — haftalık yapı desteklemeli
        # Koşul 5: 1W RSI slope > 0 — haftalık momentum yükseliyor olmalı
        # Koşul 6: ADX ≥ 20 — minimum trend gücü (V117'de bypass ediliyordu)
        # Koşul 7: RVOL ≥ 1.0 — saf hacimsizlik değil, en azından normal hacim
        is_early_awakening = (
            current_price > ema20_now                   # Fiyat EMA20 üstü
            and 45 <= rsi_1d_val <= 55                  # RSI sweet spot
            and 0.9 <= rvol_today <= 1.3                # Stealth bölgesi
            and w_rsi_val >= 50                         # V117 YENİ: 1W RSI ≥ 50
            and w_rsi_slope > 0                         # V117 YENİ: 1W RSI yükseliyor
            and adx_1d >= 20                            # V117 YENİ: Trend varlığı zorunlu
            and rvol_today >= 1.0                       # V117 YENİ: Mutlak RVOL alt sınırı
        )
        ...
        if sector_name in ["Energy", "Basic Materials", "Technology", "Healthcare"]:
            score += 4.0
            details.append(f"💎 Güvenilir Sektör Primi: {sector_name} (+4p)")
        ...
        return {
            ...
            "meta":                     {
                ...
                "1h":            h1_summary,
                "volume_regime": volume_regime_str,
                "version":       "v117.v2",                # v117.v2 YENİ — sürüm takibi
            },
...
def compute_boga_score_100(c: dict) -> float:
    ...
    if fin:
        if sector == "Real Estate":
            ...
            elif fcf < 0:   fund_pts -= 4.0

            net_m = fin.get("net_margin", 0)
            if net_m < 0:   fund_pts -= 4.0

            score += max(-8.0, min(fund_pts, 10.0))
