// BogaSmart Copilot FAQ Knowledge Service — spec böl. 25/33.
...
const TR: FaqEntry[] = buildEntries("tr", [
  ...
  { q: "İnteraktif Grafik ve BogaSmart analizi bana nasıl yardımcı olacak?", a: "Grafik detay ekranımızda sadeleştirilmiş interaktif grafik desteği sunuyoruz. BogaSmart, bu grafik üzerinde net ticaret planları (Giriş bölgesi, Hedef seviyeleri ve Stop Loss noktası) üretir. Siz bu verileri inceleyerek kararınızı verir, kendi risk toleransınıza göre pozisyon büyüklüğünüzü ayarlarsınız." },
  { q: "BogaSmart sistemi zamanla hata yapar mı? Kendini nasıl geliştiriyor?", a: "Dünyada hatasız çalışan hiçbir finansal sistem veya yapay zeka yoktur; piyasalar her zaman belirsizliklerle doludur. BogaSmart, kendi geliştirdiğimiz LLM yapısı ve makine öğrenimi altyapısı sayesinde gerçekleşen her işlem sonucunu analiz ederek kendini sürekli günceller." },
  ...
  { q: "Sistemdeki işlemlerde kaybetme olasılığım var mı?", a: "Evet, kesinlikle var. Finansal piyasalarda %100 başarı vaat eden hiçbir dürüst sistem olamaz. BogaSmart'ın tarihsel başarı oranı geçmiş verilere dayanır ve gelecekte her işlemin karla sonuçlanacağını garanti etmez. Her işlemde riskinizi sınırlamalı ve asla tek bir işleme tüm sermayenizi bağlamamalısınız." },
...
const EN: FaqEntry[] = buildEntries("en", [
  ...
  { q: "How do the Interactive Charts and BogaSmart analysis help me?", a: "BogaSmart analyzes our simplified interactive charts to map out objective trading plans, defining clear entry zones, profit targets, and stop-loss levels. You review these insights and manage your trades based on your personal risk tolerance." },
  { q: "Does BogaSmart ever make mistakes? How does it improve?", a: "Yes, it can. There is no such thing as an infallible financial system or AI; markets are inherently uncertain. BogaSmart uses a proprietary LLM framework combined with machine learning, continuously analyzing the outcome of every trade to refine its parameters." },
  ...
  { q: "Is there a risk of losing money when trading with this system?", a: "Yes, absolutely. Any system promising guaranteed profits in financial markets is not being honest. BogaSmart's historical win rates are based on past data, which does not guarantee future results. Always practice proper risk management." },
...
const ES: FaqEntry[] = buildEntries("es", [
  { q: "¿Qué es exactamente BOGASTOCK y qué puede hacer por mí?", a: "BOGASTOCK es una plataforma de tecnología financiera que utiliza inteligencia artificial (BogaSmart) y algoritmos matemáticos avanzados para escanear miles de acciones en las bolsas de Estados Unidos (NYSE, NASDAQ y AMEX). Identificamos, puntuamos y te presentamos una lista clara con las 20-30 mejores Acciones en Tendencia." },
  ...
  { q: "¿Cómo me ayudan los Gráficos Interactivos y el análisis de BogaSmart?", a: "BogaSmart analiza los patrones del gráfico para generar planes de trading claros: zona de entrada, objetivos de beneficio y nivel de stop loss. Tú gestionas tu operación según tu tolerancia al riesgo." },
  { q: "¿La inteligencia artificial BogaSmart comete errores? ¿Cómo mejora?", a: "Sí, puede cometerlos. No existe ningún sistema financiero infalible. BogaSmart cuenta con una estructura de LLM propia y aprendizaje automático, analizando constantemente el resultado de cada operación para autoajustarse." },
  ...
  { q: "¿Existe el riesgo de perder dinero operando con este sistema?", a: "Sí, por supuesto. Cualquier promesa de rentabilidad garantizada al 100% es falsa. Las estadísticas de BogaSmart se basan en datos del pasado, que no garantizan resultados futuros. Limita tu riesgo en cada operación." },
...
const FR: FaqEntry[] = buildEntries("fr", [
  { q: "Qu'est-ce que BOGASTOCK exactement et qu'est-ce que cela m'apporte ?", a: "BOGASTOCK est une plateforme fintech qui utilise l'IA (BogaSmart) et des algorithmes mathématiques avancés pour analyser des milliers d'actions sur les marchés américains (NYSE, NASDAQ, AMEX), et sélectionne pour vous les 20 à 30 meilleures Actions Tendance." },
  ...
  { q: "Comment les graphiques interactifs et l'analyse de BogaSmart m'aident-ils ?", a: "BogaSmart analyse les graphiques pour tracer des plans de trading clairs : zone d'entrée, objectifs de gains et stop-loss. Vous gérez vos trades selon votre tolérance au risque." },
  { q: "Le système BogaSmart peut-il faire des erreurs ?", a: "Oui. Il n'existe aucun système financier infaillible. BogaSmart s'appuie sur un LLM propriétaire et le machine learning, analysant en continu les résultats de chaque opération pour s'améliorer." },
  ...
  { q: "Y a-t-il un risque de perdre de l'argent avec ce système ?", a: "Oui, absolument. Toute promesse de gain garanti à 100 % est mensongère. Les statistiques de BogaSmart reposent sur des données passées, sans garantie pour l'avenir. Gérez toujours votre risque rigoureusement." },
...
const PT: FaqEntry[] = buildEntries("pt", [
  { q: "O que é exatamente a BOGASTOCK e como ela pode me ajudar?", a: "A BOGASTOCK é uma plataforma de tecnologia financeira que usa IA (BogaSmart) e algoritmos matemáticos avançados para escanear milhares de ações nas bolsas americanas (NYSE, NASDAQ e AMEX), entregando uma lista das 20-30 melhores Ações em Tendência." },
  ...
  { q: "Como o Gráfico Interativo e a análise da BogaSmart me ajudam?", a: "A BogaSmart analisa os padrões do gráfico e gera planos de operação: zona de entrada, alvos de lucro e stop loss. Você gerencia a operação de acordo com seu perfil de risco." },
  { q: "A inteligência artificial BogaSmart pode errar?", a: "Sim, pode errar. Não existe sistema financeiro infalível. A BogaSmart conta com um LLM proprietário e aprendizado de máquina, analisando constantemente os resultados de cada operação para se autoajustar." },
  ...
  { q: "Existe a chance de eu perder dinheiro operando com o sistema?", a: "Sim, existe. Qualquer promessa de 100% de acerto é mentira. As estatísticas da BogaSmart se baseiam em dados passados, sem garantia de resultados futuros. Limite seus riscos em cada operação." },
