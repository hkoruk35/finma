...
export async function evaluatePendingLogs(snapshot: AssetSnapshot) {
  ...
  for (const log of pendingLogs) {
    ...
    if (log.direction === "SHORT") {
      ...
      if (spotPrice >= log.invalidation_price) {
        newStatus = "LOST";
      ...
      else if (spotPrice <= log.entry_price - 10) {
        newStatus = "WON";
