...
export function generateAiMarketCommentary(input: AiAiInput): AiMarketCommentary {
  ...
  if (category === "commodity") {
    summary = pick(lang, {
      tr: `${ticker} spot piyasada ${fmtP(price)} seviyesinde işlem görmektedir (%${changePct > 0 ? "+" : ""}${changePct.toFixed(2)}). Fiyatın 20 günlük EMA (${fmtP(ema20)}) ve VWAP (${fmtP(vwap)}) üzerindeki konumu ${bias === "BULLISH" ? "kurumsal alım baskısının ve makro yükseliş trendinin korunduğunu gösterir." : "makro satıcıların etkisinde dengelenme aradığını göstermektedir."} ATR% %${atrPct.toFixed(2)} ile emtia volatilitesinin aktif olduğunu doğrulamaktadır.`,
      en: `${ticker} is trading at ${fmtP(price)} in spot markets (${changePct > 0 ? "+" : ""}${changePct.toFixed(2)}%). Price relative to the 20-day EMA (${fmtP(ema20)}) and VWAP (${fmtP(vwap)}) ${bias === "BULLISH" ? "indicates sustained institutional buying pressure and macro uptrend alignment." : "suggests market participants are absorbing supply below key dynamic resistance."} ATR% at ${atrPct.toFixed(2)}% reflects active commodity volatility.`,
      es: `${ticker} cotiza a ${fmtP(price)} en los mercados al contado (${changePct > 0 ? "+" : ""}${changePct.toFixed(2)}%). El precio con respecto a la EMA20 (${fmtP(ema20)}) y VWAP (${fmtP(vwap)}) ${bias === "BULLISH" ? "indica una presión compradora institucional sostenida y alineación de tendencia macro." : "sugiere consolidación bajo resistencia dinámica."} El ATR% de ${atrPct.toFixed(2)}% refleja volatilidad activa.`,
      fr: `${ticker} se négocie à ${fmtP(price)} sur les marchés au comptant (${changePct > 0 ? "+" : ""}${changePct.toFixed(2)}%). Le prix par rapport à l'EMA20 (${fmtP(ema20)}) et au VWAP (${fmtP(vwap)}) ${bias === "BULLISH" ? "indique une pression acheteuse institutionnelle soutenue et un alignement haussier macro." : "suggère une absorption de l'offre sous résistance."} L'ATR% à ${atrPct.toFixed(2)}% reflète une volatilité active.`,
      pt: `${ticker} está negociando a ${fmtP(price)} nos mercados à vista (${changePct > 0 ? "+" : ""}${changePct.toFixed(2)}%). O preço em relação à EMA20 (${fmtP(ema20)}) e VWAP (${fmtP(vwap)}) ${bias === "BULLISH" ? "indica pressão compradora institucional sustentada e alinhamento de tendência de alta." : "sugere consolidação sob resistência dinâmica."} O ATR% em ${atrPct.toFixed(2)}% reflete volatilidade ativa.`,
    });
  } else if (category === "forex") {
    summary = pick(lang, {
      tr: `${ticker} paritesi ${fmtP(price)} seviyesinde seyretmektedir. Gün içi VWAP (${fmtP(vwap)}) ve pivot seviyesi (${fmtP(pivotP)}) etrafındaki fiyatlama, ${bias === "BULLISH" ? "alıcıların üst üste seanslarda kontrolü elinde tuttuğuna işaret etmektedir." : "satıcı ağırlıklı momentumun devam ettiğine işaret etmektedir."} RSI (${rsi.toFixed(0)}) dengeli ivmeyi desteklemektedir.`,
      en: `${ticker} currency pair is trading at ${fmtP(price)}. Price action around intraday VWAP (${fmtP(vwap)}) and Daily Pivot (${fmtP(pivotP)}) ${bias === "BULLISH" ? "indicates buyers maintaining control across sessions." : "signals prevailing downside order flow momentum."} RSI at ${rsi.toFixed(0)} reflects balanced momentum dynamics.`,
      es: `El par ${ticker} cotiza a ${fmtP(price)}. La acción del precio alrededor del VWAP intradía (${fmtP(vwap)}) y el Pívot Diario (${fmtP(pivotP)}) ${bias === "BULLISH" ? "indica que los compradores mantienen el control en las sesiones." : "señala impulso vendedor predominante."} El RSI en ${rsi.toFixed(0)} refleja dinámica equilibrada.`,
      fr: `La paire ${ticker} évolue à ${fmtP(price)}. L'action des prix autour du VWAP intrajournalier (${fmtP(vwap)}) et du Pivot Journalier (${fmtP(pivotP)}) ${bias === "BULLISH" ? "indique que les acheteurs gardent le contrôle." : "signale un momentum vendeur prédominant."} Le RSI à ${rsi.toFixed(0)} confirme la dynamique.`,
      pt: `O par ${ticker} é negociado a ${fmtP(price)}. A ação do preço ao redor do VWAP intradiário (${fmtP(vwap)}) e Pivô Diário (${fmtP(pivotP)}) ${bias === "BULLISH" ? "indica compradores mantendo o controle." : "sinaliza impulso vendedor predominante."} O RSI em ${rsi.toFixed(0)} reflete dinâmica equilibrada.`,
    });
  } else if (category === "crypto") {
    summary = pick(lang, {
      tr: `${ticker} 24 saatlik kripto likidite akışında ${fmtP(price)} seviyesindedir (%${changePct > 0 ? "+" : ""}${changePct.toFixed(2)}). 20 günlük EMA (${fmtP(ema20)}) ve 200 günlük trend çizgisi (${fmtP(ema200)}) ${bias === "BULLISH" ? "zincir içi ve teknik birikim yapısını doğrulamaktadır." : "likidite arayışının devam ettiğini göstermektedir."} Wyckoff birikim skoru (${wyckoffScore}/100) piyasa yapısını özetler.`,
      en: `${ticker} is trading at ${fmtP(price)} across 24/7 crypto liquidity pools (${changePct > 0 ? "+" : ""}${changePct.toFixed(2)}%). Technical alignment with the 20-day EMA (${fmtP(ema20)}) and 200-day baseline (${fmtP(ema200)}) ${bias === "BULLISH" ? "confirms ongoing structural accumulation." : "reflects liquidity hunting under key moving averages."} Wyckoff structure score is ${wyckoffScore}/100.`,
      es: `${ticker} cotiza a ${fmtP(price)} en los pools de liquidez cripto 24/7 (${changePct > 0 ? "+" : ""}${changePct.toFixed(2)}%). La alineación técnica con la EMA20 (${fmtP(ema20)}) y la EMA200 (${fmtP(ema200)}) ${bias === "BULLISH" ? "confirma acumulación estructural continua." : "refleja búsqueda de liquidez."} Puntuación Wyckoff: ${wyckoffScore}/100.`,
      fr: `${ticker} se négocie à ${fmtP(price)} sur les pools de liquidité crypto 24/7 (${changePct > 0 ? "+" : ""}${changePct.toFixed(2)}%). L'alignement technique avec l'EMA20 (${fmtP(ema20)}) et l'EMA200 (${fmtP(ema200)}) ${bias === "BULLISH" ? "confirme une accumulation structurelle en cours." : "reflète une recherche de liquidité."} Score Wyckoff : ${wyckoffScore}/100.`,
      pt: `${ticker} está negociando a ${fmtP(price)} nos pools de liquidez cripto 24/7 (${changePct > 0 ? "+" : ""}${changePct.toFixed(2)}%). O alinhamento técnico com a EMA20 (${fmtP(ema20)}) e a EMA200 (${fmtP(ema200)}) ${bias === "BULLISH" ? "confirma acumulação estrutural contínua." : "reflete busca por liquidez."} Pontuação Wyckoff: ${wyckoffScore}/100.`,
    });
  } else {
    summary = pick(lang, {
  ...
  const liquidityVolume = pick(lang, {
    tr: category === "forex"
      ? `Döviz likidite akışı: Gün içi hacim ve spread dengesi standart seans aralığında. RVOL: ${rvol.toFixed(2)}x.`
      : `Göreceli Hacim (RVOL): ${rvol.toFixed(2)}x. ${rvol >= 1.5 ? "Piyasa katılımı ortalamanın üzerinde ve kurumsal emir akışı aktif." : "Hacim teyidi ortalama seviyelerde, pozisyon büyüklüğü risk yönetimine göre ayarlanabilir."}`,
    en: category === "forex"
      ? `Forex liquidity flow: Intraday order flow and spread dynamics are in standard session bounds. RVOL: ${rvol.toFixed(2)}x.`
      : `Relative Volume (RVOL): ${rvol.toFixed(2)}x. ${rvol >= 1.5 ? "Market participation is above average with active institutional order flow." : "Volume confirmation is moderate; position sizing can be managed according to risk preference."}`,
    es: category === "forex"
      ? `Flujo de liquidez Forex: El flujo de órdenes intradía está en rangos estándar. RVOL: ${rvol.toFixed(2)}x.`
      : `Volumen Relativo (RVOL): ${rvol.toFixed(2)}x. ${rvol >= 1.5 ? "Participación de mercado por encima del promedio con flujo institucional activo." : "Confirmación de volumen moderada; gestione la posición según su riesgo."}`,
    fr: category === "forex"
      ? `Flux de liquidité Forex : Flux d'ordres intrajournaliers dans les limites standards. RVOL: ${rvol.toFixed(2)}x.`
      : `Volume Relatif (RVOL) : ${rvol.toFixed(2)}x. ${rvol >= 1.5 ? "Participation au marché supérieure à la moyenne avec flux institutionnel actif." : "Confirmation de volume modérée ; gérez la taille de position selon votre risque."}`,
    pt: category === "forex"
      ? `Fluxo de liquidez Forex: Fluxo de ordens intradiárias dentro dos limites padrão. RVOL: ${rvol.toFixed(2)}x.`
      : `Volume Relativo (RVOL): ${rvol.toFixed(2)}x. ${rvol >= 1.5 ? "Participação de mercado acima da média com fluxo institucional ativo." : "Confirmação de volume moderada; ajuste o tamanho da posição de acordo com o risco."}`,
  });
