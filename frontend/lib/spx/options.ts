...
export function buildChain(input: ChainInput): OptionQuote[] {
  const { spot, vix, minutesLeft, type, targetPrice } = input;
  const isCall = type === "CALL";
  const atm = Math.round(spot / 5) * 5;
  const offsets = input.offsets ?? [0, 5, 10, 15, 20, 25, 30];
...
export function simulateRunners(input: RunnerInput): RunnerSimulation {
  ...
  const spot = entryFrame.spxPrice;
  ...
  const path = frames.slice(entryIdx).map((f) => {
    ...
    const iv = impliedVolFor(vix, f.spxPrice, strike);
    return {
      frame: f,
      premium: priceOption(f.spxPrice, strike, minutesLeft, iv, isCall).price,
    };
  });
  ...
  const models: RunnerResult[] = MODELS.map((def) => {
    ...
    for (let i = 1; i < path.length; i++) {
      ...
      if (def.id === "A") {
        ...
        const px = path[i].frame.spxPrice;
