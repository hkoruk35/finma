...
import type { Bar, EsLevels, SpxLevels, TrendStructure } from "./types";
import {
  RTH_OPEN_MIN,
  aggregate,
  barsOnDate,
...
export function buildSessionSlices(
  ...
  const esRth = barsOnDate(esBars, sessionDate);
  const spxRth = barsOnDate(spxBars, sessionDate);
  const esOn = overnightBars(esBars, sessionDate, prevDate);
  const esPm = premarketBars(esBars, sessionDate);
...
export function computeEsLevels(
  esBars: Bar[],
  ...
  if (prevDate) {
    const prev = barsOnDate(esBars, prevDate);
    if (prev.length) {
  ...
  if (rth.length) {
    ...
    if (price > vwap + 0.5) result.priceVsVwap = "ABOVE";
    else if (price < vwap - 0.5) result.priceVsVwap = "BELOW";
    else result.priceVsVwap = "AT";

    if (i >= 5) {
      const delta = vwaps[i] - vwaps[i - 5];
      if (delta > 0.25) result.vwapSlope = "RISING";
      else if (delta < -0.25) result.vwapSlope = "FALLING";
    }
...
export function computeSpxLevels(
  spxBars: Bar[],
  slices: SessionSlices,
  upToIndex?: number
): SpxLevels {
  ...
  if (prevDate) {
    const prev = barsOnDate(spxBars, prevDate);
    if (prev.length) {
      result.pdh = round2(Math.max(...prev.map((b) => b.high)));
      result.pdl = round2(Math.min(...prev.map((b) => b.low)));
      result.pdc = round2(prev[prev.length - 1].close);
