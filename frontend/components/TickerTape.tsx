...
export default function TickerTape({ data, indices }: Props) {
  ...
  return (
    <div className="w-full bg-[#0a0e17] border-b border-[#1e2a3a] overflow-x-auto no-scrollbar scroll-smooth">
      <div className="ticker-tape flex items-center gap-8 py-2 px-4 whitespace-nowrap min-w-max">
        {doubled.map((item, i) => (
          <div key={i} className="flex items-center gap-2 text-xs shrink-0">
            <span className="text-white font-medium uppercase tracking-wider text-[10px]">{item.label}</span>
            <span className="font-mono text-white/70 text-xs">{item.value.toFixed(2)}</span>
            <span
