    ...
    <div className={fullPage ? "max-w-5xl mx-auto px-4 py-8" : "p-4"}>
      ...
      {data.warnings?.length > 0 && (
        <div className="flex gap-2 mb-3 flex-wrap">
          {data.warnings.map((w) => (
            <div key={w} className="bg-amber-500/10 border border-amber-500/40 text-amber-400 text-[10px] px-2.5 py-1 rounded">
              ⚠️ {w}
            </div>
          ))}
        </div>
      ...
      {data.activeSignals?.length > 0 && (
        <div className="flex gap-1.5 mb-3 flex-wrap items-center">
          <span className="text-[10px] text-white/40 font-bold tracking-wider mr-1">{t.activeSignals}:</span>
          {data.activeSignals.map((s) => (
            <span key={s} className="bg-green-900/30 border border-green-700/50 text-green-400 text-[10px] px-2 py-0.5 rounded-full">
              ✓ {s}
            </span>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="bg-[#111620] border border-[#253347] rounded-lg p-3.5">
          <div className="text-[10px] text-white/40 uppercase tracking-widest font-bold mb-2.5">{t.technicalCard}</div>
          {[
            ...
            <div key={label} className="flex justify-between mb-1 text-[10px]">
        ...
        <div className="bg-[#111620] border border-[#253347] rounded-lg p-3.5">
          <div className="text-[10px] text-white/40 uppercase tracking-widest font-bold mb-2.5">{t.marketCard}</div>
          {[
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              <div key={p.label} className={`border rounded-md py-2 text-center ${p.bg}`}>
                <div className={`text-[9px] font-bold tracking-wider ${p.color}`}>{p.label}</div>
                <div className={`text-sm font-mono font-extrabold mt-0.5 ${p.color}`}>{p.value}</div>
              </div>
            ))}
          </div>
          <div className="bg-[#111620] border border-[#253347] rounded-md py-2 text-center">
            <div className="text-[10px] text-white/40 font-semibold">{t.riskPct}</div>
            <div className="text-sm font-bold text-amber-400">{fmt(data.tradePlan.stop.pct)}%</div>
          </div>
          {!fullPage && (
            <a href={permalinkHref} className="text-center text-[11px] font-bold text-blue-400 border border-blue-500/40 bg-blue-500/10 rounded-md py-1.5 hover:bg-blue-500/20 transition-colors">
              {t.permalink} ↗
            </a>
          )}
        </div>
      ...
      <div className="mt-3 bg-[#111620] border border-[#253347] rounded-lg p-3.5">
        <div className="text-[10px] text-white/40 uppercase tracking-widest font-bold mb-2.5">{t.scoreCard}</div>
        <div className="grid grid-cols-3 gap-3">
          {scoreBars.map(({ label, score, color }) => (
            <div key={label}>
              <div className="flex justify-between mb-1 items-center">
                <span className="text-[10px] text-white/50 font-semibold">{label}</span>
                <span className="text-xs font-mono font-extrabold" style={{ color }}>{score}</span>
              </div>
