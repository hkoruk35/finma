        ...
        {picks.slice(0, 5).map((item, idx) => {
          ...
          return (
            <div key={item.ticker} className={`relative flex flex-col ${idx > 0 ? 'border-t border-white/15 pt-8 md:border-t-0 md:pt-0' : ''}`}>
              ...
              >
                {/* Rank badge */}
                <div className={`absolute top-0 right-0 ${minimal ? 'w-8 h-8 text-[10px]' : 'w-10 h-10 text-sm'} rounded-bl-2xl flex items-center justify-center font-medium text-white shadow-2xl bg-gradient-to-br from-[#3b82f6] to-[#1e3a8a] z-20`}>
                  #{idx + 1}
                </div>

                {/* Ticker & Company */}
                <div className={`${minimal ? 'mb-3' : 'mb-4'} min-h-[60px]`}>
                  <div className={`${minimal ? 'text-3xl md:text-xl' : 'text-4xl md:text-2xl'} font-medium text-white transition-colors tracking-tighter uppercase ${isLocked ? 'blur-[12px] opacity-40 select-none' : 'group-hover:text-[#3b82f6]'}`}>
                    ...
                    [
                      ...
                      <div key={row.label} className={`flex items-center justify-between px-3 py-2 ${i < arr.length - 1 ? 'border-b border-white/5' : ''}`}>
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[9px] text-white font-medium uppercase tracking-wider">{row.label}</span>
                        </div>
                ...
                <div className="grid grid-cols-6 gap-1 mt-2.5 w-full">
                  {Object.entries(LANG_CONFIG).map(([l, cfg]) => {
                    ...
                    return (
                      ...
                      >
                        <span className="text-[7px] font-medium text-white group-hover/lang:text-[#3b82f6] transition-colors leading-none mb-0.5 uppercase tracking-tighter">
                          {countryPart}
                        </span>
                        <span className="text-[12px] font-medium text-[#00d2ff] group-hover/lang:text-white transition-colors leading-none tracking-tight">
