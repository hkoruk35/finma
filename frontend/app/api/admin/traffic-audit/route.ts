...
import { classifySource, sessionStage, type TrafficSession } from "@/lib/trafficAudit";
...
export async function GET(req: NextRequest) {
  ...
  const sources = Array.from(sourceMap.entries())
    .map(([label, rows]) => ({
      ...
      withTwclid: label === "X / paid_social" ? rows.filter((r) => !!r.twclid).length : null,
    }))
  ...
  // ── Visitor Detail (point 11) — flags (point 10) ────────────────────────
  const visitors = sessions.slice(0, 500).map((s) => ({
    ...
    suspectedAutomation:
      s.suspected_bot_ua ||
      (!s.page_loaded && s.first_seen < Date.now() - 5 * 60 * 1000) || // request var, page_loaded 5dk+ gecti hala yok
      (s.page_loaded && !s.active_5s && s.first_seen < Date.now() - 5 * 60 * 1000),
  }));
