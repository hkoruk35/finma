...
import { getDailyArchiveDates } from "@/lib/indexSnapshots";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  for (const l of INDEX_LOCALES) {
    languages[l] = `https://bogastock.com/global/${l}/${indexSlug}/daily`;
  }

  return {
    title: `${name} — ${t.dailyArchive} | BOGASTOCK`,
    description: `${name} ${t.dailyArchive}`,
    alternates: {
      canonical: `https://bogastock.com/global/${locale}/${indexSlug}/daily`,
      languages,
    },
  };
}

export default async function IndexDailyArchivePage({ params }: Props) {
  const { locale, indexSlug } = await params;
  const indexDef = getIndexBySlug(indexSlug);
  ...
  const t = copy[locale].indices;
  const name = indexDef.names[locale];
  const dates = await getDailyArchiveDates(indexDef.symbol, 60);
    ...
    <div lang={locale} className="min-h-screen flex flex-col bg-[#0a0e17]">
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-6 py-6">
        <nav className="flex items-center gap-2 text-[10px] font-medium text-slate-500 uppercase tracking-widest mb-4 flex-wrap">
          ...
          <span className="opacity-30">/</span>
          <span className="text-white">{t.dailyArchive}</span>
        </nav>
        ...
        {dates.length > 0 ? (
          <div className="flex flex-wrap gap-2">
            {dates.map((date) => (
              <Link
                key={date}
                href={`/global/${locale}/${indexSlug}/daily/${date}`}
