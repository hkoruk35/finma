    ...
    <div className="h-screen bg-[#080c14] text-white flex overflow-hidden">
      ...
      >
        ...
        {searchHistory.length > 0 && (
          <div className="flex-1 overflow-y-auto px-3">
            ...
            {/* Trending */}
            <div className="text-[10px] font-black text-[#64748b] uppercase tracking-widest mb-2 mt-6">Trending</div>
            <div className="space-y-1">
              {TRENDING_SEARCHES.slice(0, 5).map((q, i) => (
                <button
                  key={i}
                  onClick={() => send(q)}
                  className="w-full text-left text-xs p-2 rounded hover:bg-[#1e2a3a] transition-colors text-[#94a3b8] truncate"
                  title={q}
                >
                  {q.slice(0, 30)}
                </button>
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-h-0 max-w-4xl w-full mx-auto px-4 pt-6 pb-4">
          {/* Title */}
          <div className="mb-5 text-center shrink-0 flex items-center justify-between">
            <button
            ...
            <div className="flex-1">
              ...
              <p className="text-[#64748b] text-xs">Financial markets | Technical analysis | Global trading</p>
            </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {m.followUp.map((q, j) => (
                            <button
                              key={j}
                              onClick={() => send(q)}
                              className="text-[10px] px-2 py-1 rounded border border-[#1e2a3a] text-[#cbd5e1] hover:border-[#3b82f6]/40 hover:text-white transition-all bg-[#0d1117] hover:bg-[#1e2a3a]"
                            >
                              {q}
                            </button>
                          ))}
          ...
          <div className="shrink-0 pt-3">
            ...
            <div className="flex gap-2 items-end bg-[#0d1117] border border-[#1e2a3a] rounded-2xl px-4 py-3 focus-within:border-[#3b82f6]/50 transition-colors shadow-2xl shadow-black/40">
              <textarea
                ...
                onKeyDown={handleKey}
                placeholder="Ask about stocks, commodities, crypto... type /swing, /top5, /analiz..."
                rows={1}
            ...
            <p className="text-center text-[10px] text-[#334155] mt-2">Financial markets expert • Always verify before trading</p>
          </div>
        </div>
      </div>
    </div>
