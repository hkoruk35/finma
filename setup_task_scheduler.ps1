...
Register-ScheduledTask -TaskName $TaskName -Action $Action -Trigger $Trigger -Settings $Settings -Principal $Principal -Description "BOGA AI Daily Bot - run_all_bots.py (v115.0) weekdays 18:00 NY (EDT)" -Force | Out-Null
...
if ($Task) {
    ...
    Write-Host "Schedule: Weekdays 18:00 NY (EDT)"
