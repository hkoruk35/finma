...
| `spyengine_v4_forecasts` | `{sessions: Record<date, {date, updatedAt, snapshots: {at,remainingMin,low,high,mid}[], actualClose}>}` | SPY Engine's OWN algorithmic day-close forecast band + hit-rate tracking (`app/api/admin/spyengine/v2/forecast/route.ts`). Not `ALLOWED_KEYS`-gated — read/written directly via `supabaseAdmin`. |
| `spyengine_manual_forecast` | `{sessions: Record<date, {date, createdAt, updatedAt, points: {time,price}[], imageUrl}>}` | User's own manual 5-minute SPY price forecast (09:30–16:00 ET), entered by hand or via image (Claude vision-extracted) in the "Daily Forecast" tab — compared against realized 5m closes, archived per day. Completely separate from `spyengine_v4_forecasts` (the engine's own algorithmic forecast) — this one has zero effect on engine decisions, it's purely to test the user's own forecasting method. `app/api/admin/spyengine/v2/manual-forecast/route.ts` + `lib/spyengine/manualForecastStore.ts`. Not `ALLOWED_KEYS`-gated. |
| `spyengine_option_decision` | `{generatedAt, spot, expiry, isZeroDte, yearsToExpiry, riskFreeRate, contracts: {strike, call: {lastPrice,bid,ask,impliedVolatility,greeks}, put: {...}, premiumCurve: {targetPrice,callPremium,putPremium}[]}[]}` | SPY 0DTE (or nearest-available) Black-Scholes Greeks + a per-strike premium-sensitivity curve, written by `spy_0dte_options_sync.py` (repo root) on the `BOGA_AI_Options_Scanner` schedule (11:00 & 15:30 NY, via `run_options_scanner.py`) — see `tasks/active/013`. **Deliberately independent of `opsiyon242.py`**: that script's hard entry gate (RVOL≥1.5 etc.) and DTE range (1–30, excludes 0DTE) make it unsuitable for an always-on SPY 0DTE view without risking either suppressing SPY most of the time or loosening the gate/DTE range for every other scanned ticker. IV always comes from Yahoo's own chain (never HV-substituted); a contract is omitted if no IV is available. Read via `lib/spyengine/optionDecisionStore.ts` (`readOptionDecision()`, treats data older than 6h as stale → `null`). Not `ALLOWED_KEYS`-gated — read/written directly via `supabaseAdmin`/raw Supabase REST. |
