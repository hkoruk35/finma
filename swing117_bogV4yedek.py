...
# ================================================================
# ================================================================
# SECTION 12.5: PERSİSTAN ADAY HAVUZU (v4.0 Hybrid — candidate_pool.json)
# ================================================================
# ================================================================
...
# Dynamic Relaxation Constants
SCAN_MIN_AVG_VOLUME = ATMACA_MIN_AVG_VOLUME
SCAN_MIN_DOLLAR_VOLUME = ATMACA_MIN_DOLLAR_VOLUME
SCAN_MIN_MARKET_CAP = ATMACA_MIN_MARKET_CAP
SCAN_PRICE_MIN = PRICE_MIN
...
# ================================================================
# 🔹 STRATEJİYE ÖZEL 15M TETİKLEME MOTORU (v4.0 Hybrid)
# ================================================================

def layer4_entry_trigger_15m_hybrid(c: dict, df_15m: pd.DataFrame, relaxation_level: int = 0) -> dict:
    out = {
    ...
    try:
        ...
        prev_c = float(cl.iloc[-2]);  prev_o = float(o.iloc[-2])

        v20 = float(v.iloc[-21:-1].mean())
        rvol_15m = float(v.iloc[-1]) / v20 if v20 > 0 else 0.0

        is_green = curr_c > curr_o and curr_c > prev_c
        rsi_15_series = RSIIndicator(cl, 14).rsi()
        rsi_15 = float(rsi_15_series.iloc[-1]) if not rsi_15_series.empty else 50.0

        # Tetik parametrelerini gevşetme seviyesine göre ayarla
        rvol_threshold = max(0.5, 1.2 - (0.35 * relaxation_level))
        ema_touch_band = 0.50 + (0.35 * relaxation_level)
        rsi_reset_low = max(30, 40 - (5 * relaxation_level))
        rsi_reset_high = min(80, 65 + (5 * relaxation_level))

        # ── 1. SQUEEZE & BREAKOUT SİSTEMLERİ İÇİN BREAKOUT TETİĞİ ──
        if sys_name in ("SQUEEZE", "BREAKOUT", "TREND_CONT"):
            # Son 20 mumun en yükseğini yukarı kırma kontrolü
            recent_highs = h.iloc[-21:-1].dropna()
            if len(recent_highs) >= 15:
                breakout_level = float(recent_highs.max())
                if is_green and curr_c > breakout_level and rvol_15m >= rvol_threshold:
                    out["triggered"] = True
        ...
        # ── 2. SPRING SİSTEMİ İÇİN OVERSOLD BOUNCE TETİĞİ ──
        if sys_name == "SPRING":
            # Son 12 mum içinde aşırı satım (RSI < 30) oldu mu ve şimdi dönüyor mu?
            rsi_window = rsi_15_series.tail(12).dropna()
            if not rsi_window.empty and rsi_window.min() < 35:
                # RSI 15m'de aşırı satımdan yukarı sekiyor
                macd_obj = MACD(cl)
                macd_hist = macd_obj.macd_diff()
                macd_turned_up = float(macd_hist.iloc[-1]) > float(macd_hist.iloc[-2]) if len(macd_hist) >= 2 else True
                if is_green and (rsi_15 > 35) and macd_turned_up:
                    out["triggered"] = True
        ...
        # ── 3. EMA_CROSS SİSTEMİ İÇİN 15M / 1H EMA CROSSOVER ──
        if sys_name == "EMA_CROSS":
            ema9_15 = EMAIndicator(cl, 9).ema_indicator()
            ema20_15 = EMAIndicator(cl, 20).ema_indicator()
            if len(ema9_15) >= 2 and len(ema20_15) >= 2:
                fresh_cross = float(ema9_15.iloc[-1]) > float(ema20_15.iloc[-1]) and float(ema9_15.iloc[-2]) <= float(ema20_15.iloc[-2])
                if fresh_cross and is_green:
                    out["triggered"] = True
                    out["trigger_type"] = "15M_EMA_CROSS"
                    out["rvol_15m"] = round(rvol_15m, 2)
                    out["state_note"] = f"EMA Cross tetiği yakalandı: 15m grafik üzerinde EMA9/20 Golden Cross"
                    return out

        # ── 4. PULLBACK & DİĞER DURUMLAR İÇİN STANDART PULLBACK TETİĞİ (EMA20 teması) ──
        ema20_15 = EMAIndicator(cl, 20).ema_indicator()
        ...
        search = h.iloc[-31:-1]
        if not search.empty:
            swing_pos = int(np.argmax(search.values))
            swing_idx = len(df_15m) - 31 + swing_pos
            swing_high_val = float(h.iloc[swing_idx])

            pull_start = swing_idx + 1
            pull_end = len(df_15m) - 1
            n_pull = pull_end - pull_start
            out["pullback_candles"] = n_pull

            if PULLBACK_MIN_CANDLES <= n_pull <= PULLBACK_MAX_CANDLES + (3 * relaxation_level):
                pull_low = float(l.iloc[pull_start:pull_end].min()) if pull_end > pull_start else curr_l
                depth_atr = (swing_high_val - pull_low) / atr_15
                out["pullback_depth_atr"] = round(depth_atr, 2)

                if 0.3 <= depth_atr <= 5.5:
                    # EMA20 teması kontrol et
                    ema_touch = False
                    for idx in range(pull_start, pull_end + 1):
                        if idx >= len(ema20_15):
                            continue
                        ema_v = float(ema20_15.iloc[idx])
                        if not pd.isna(ema_v) and float(l.iloc[idx]) <= ema_v + (ema_touch_band * atr_15):
                            ema_touch = True
                            break
                    out["ema_touch"] = ema_touch

                    # RSI reset kontrol et
                    rsi_window = rsi_15_series.iloc[pull_start:pull_end + 1].dropna()
                    rsi_min = float(rsi_window.min()) if not rsi_window.empty else rsi_15
                    rsi_reset = rsi_reset_low <= rsi_min <= rsi_reset_high
                    out["rsi_reset"] = rsi_reset

                    if ema_touch and rsi_reset:
                        # Resume mumu teyit
                        ...
                        pull_recent_high = float(h.iloc[max(pull_start, pull_end - 3):pull_end + 1].max())
                        is_breakout_candle = (
                            is_green and body_ratio >= 0.45
                            and (curr_c > prev_h) and (curr_c >= pull_recent_high)
                        )

                        above_ema = curr_c > float(ema20_15.iloc[-1])
                        if above_ema and (is_engulfing or is_breakout_candle) and rvol_15m >= rvol_threshold:
                            out["triggered"] = True
        ...
        out["state_note"] = "EMA20 temaslı / tetik bekliyor"
        return out
    except Exception as e:
...
# ================================================================
# 🔹 ENTRY CHECK VE TETİK GEVŞETME DÖNGÜSÜ (v4.0 Hybrid)
# ================================================================
...
async def run_entry_check_pass():
    ...
    # O gün kaç işlem açıldığını kontrol et
    today_str = now_ny.strftime("%Y-%m-%d")
    entered_today_count = sum(1 for c in pool.get("swing_candidates", []) 
                             if c.get("entry_status") == "ENTERED" and c.get("entered_at", "").startswith(today_str))
    
    logging.info(f"[ENTRY_CHECK] {len(pending)} bekleyen aday kontrol ediliyor... Bugün açılan işlem: {entered_today_count}")
    updated_any = False

    # Tetik gevşetme seviyeleri
    relaxation_level = 0
    # Eğer bugün 2'den az işlem açıldıysa ve hala bekleyen adaylarımız varsa gevşetebiliriz
    if entered_today_count < 2:
        # En fazla 2 seviyeli gevşetme
        relaxation_level = 2 - entered_today_count
    ...
    for entry in pending:
        ...
        try:
            ...
            # Adayın orijinal selection_system bilgisini al
            last_pick = entry.get("last_pick") or {}
            selection_system = last_pick.get("selection_system", "DEFAULT")
            entry_c_mock = {"ticker": ticker, "selection_system": selection_system}

            # Hibrit tetikleme motorunu çalıştır
            trig = layer4_entry_trigger_15m_hybrid(entry_c_mock, df_15m, relaxation_level=relaxation_level)
            ...
            zones = calculate_support_resistance_1h(
                ...
                f"{selection_system} ({trig['trigger_type']})", df_15m
            )
            entry["entry_status"] = "ENTERED"
            entry["entry_zone"] = {
                "low": zones.get("buy_zone", {}).get("low", 0.0),
            ...
            logging.info(f"🎯 [ENTRY_CHECK] {ticker} ({selection_system}): Giriş bölgesi yakalandı (Gevşetme Lvl: {relaxation_level}) → {entry['entry_zone']}")

            # Telegram bildirimi gönder
            await send_telegram_message(
                f"🎯 <b>GİRİŞ ZONE YAKALANDI ({selection_system}): {ticker}</b>\n"
                f"Buy Zone: {entry['entry_zone']['low']:.2f} - {entry['entry_zone']['high']:.2f}\n"
                f"<i>{trig.get('state_note','')}</i>"
            )
            
            # Gevşetme seviyesini tekrar değerlendir
            if entered_today_count >= 2:
                relaxation_level = 0
            else:
                relaxation_level = 2 - entered_today_count
...
# ================================================================
# 🔹 MASTER HYBRID SCANNER (v4.0 — DİNAMİK GEVŞETME DÖNGÜLÜ)
# ================================================================

async def scan_top_stocks(mode: str = "FULL_SCAN"):
    if mode == "ENTRY_CHECK":
        logging.info("▶ ENTRY_CHECK modu — PENDING swing adayları için tetik kontrolü.")
    ...
    # ── STEP 2: UNIVERSE PREPARATION ──
    # Universe haftalık cache'li
    ...
    logging.info(f"📋 Taranacak hisse adeti: {len(tickers_to_scan)}")

    cooldown_map = load_signal_history_dates(days=COOLDOWN_DAYS)
    
    # ── DİNAMİK GEVŞETME DÖNGÜSÜ (FULL_SCAN için) ──
    # Watchlist'te en az 3 ve Swing Adaylarında en az 3 yeni hisse çıkana kadar filtreler gevşetilir.
    global ATMACA_MIN_AVG_VOLUME, ATMACA_MIN_DOLLAR_VOLUME, ATMACA_MIN_MARKET_CAP, PRICE_MIN
    ...
    while relaxation_level <= max_relaxation:
        # Gevşetme seviyelerine göre filtreleri ayarla
        if relaxation_level > 0:
            ATMACA_MIN_AVG_VOLUME = int(SCAN_MIN_AVG_VOLUME * (1.0 - 0.15 * relaxation_level))
            ATMACA_MIN_DOLLAR_VOLUME = int(SCAN_MIN_DOLLAR_VOLUME * (1.0 - 0.15 * relaxation_level))
            ATMACA_MIN_MARKET_CAP = int(SCAN_MIN_MARKET_CAP * (1.0 - 0.15 * relaxation_level))
            PRICE_MIN = max(5.0, SCAN_PRICE_MIN - (2.0 * relaxation_level))
            logging.info(f"⚠️ GEVŞETME DERECE {relaxation_level}: Min Vol:{ATMACA_MIN_AVG_VOLUME} | Min DV:{ATMACA_MIN_DOLLAR_VOLUME} | Min Price:{PRICE_MIN}")

        # ── STEP 3: PARALLEL ANALYSIS (apply_atmaca_filters) ──
        ...
        async def sem_analyze(ticker: str):
            ...
            async with semaphore:
                ...
                try:
                    ...
                    logging.debug(f"❌ {ticker} filtreleme hatası: {e}")
                    return None

        tasks = [sem_analyze(t) for t in tickers_to_scan]
        raw_results = await asyncio.gather(*tasks)
        candidates = [r for r in raw_results if r is not None]
        logging.info(f"[L1-L2] Gevşetme Seviyesi {relaxation_level} Geçen: {len(candidates)} hisse")
        ...
        # En iyi 30 hisseyi derin analize al (Derin Analiz Limiti)
        top_analysis_limit = min(30, len(candidates_ranked))
        top_analysis_set = candidates_ranked[:top_analysis_limit]
        logging.info(f"🏆 Top {len(top_analysis_set)} hisse derin analize alınıyor...")

        # ── STEP 5: LAYER 3 — DEEP ANALYSIS (Top 25-30) ──
        async def fetch_heavy_data(c: dict):
            ...
            try:
                ...
                if catalyst.get('has_catalyst'):
                    c["score"] += catalyst['score']
                    c["details"].extend(c.get("details", []) + catalyst['reasons'])
                    c["catalyst_data"] = catalyst
                    c["pfi"] = catalyst['score']
                ...
                if opt.get('bullish'):
                    c["score"] += opt.get('score', 0)
                    c["details"].extend(opt.get('details', []))
                    c["opt_sentiment"] = opt

            except Exception as e:
                logging.debug(f"⚠️ {ticker} Derin Analiz hatası: {e}")

        sem_k3 = asyncio.Semaphore(6)
        async def sem_heavy(c):
            async with sem_k3:
                await asyncio.sleep(random.uniform(0.1, 0.3))
                await fetch_heavy_data(c)

        await asyncio.gather(*(sem_heavy(c) for c in top_analysis_set))
        ...
        # Swing & Watchlist Dağılımı
        # İlk 10 hisse -> Swing, Sonraki 10 hisse -> Watchlist
        top_signals = top_candidates[:TOP_SIGNAL_PICKS]
        top_watch = top_candidates[TOP_SIGNAL_PICKS:TOP_SIGNAL_PICKS + TOP_WATCHLIST_PICKS]

        # Kaç yeni aday bulduğumuzu kontrol edelim
        new_swing_found = sum(1 for c in top_signals if c["ticker"] not in existing_tickers)
        new_watch_found = sum(1 for c in top_watch if c["ticker"] not in existing_tickers)
        
        logging.info(f"🔍 Deneme (Gevşetme: {relaxation_level}): Yeni Swing:{new_swing_found} (hedef:3) | Yeni Watchlist:{new_watch_found} (hedef:3)")

        if (new_swing_found >= 3 and new_watch_found >= 3) or relaxation_level == max_relaxation:
            final_top_signals = top_signals
            final_top_watch = top_watch
            break
        else:
            relaxation_level += 1

    # Orijinal filtre limitlerini geri yükle
    ATMACA_MIN_AVG_VOLUME = SCAN_MIN_AVG_VOLUME
    ATMACA_MIN_DOLLAR_VOLUME = SCAN_MIN_DOLLAR_VOLUME
    ATMACA_MIN_MARKET_CAP = SCAN_MIN_MARKET_CAP
    PRICE_MIN = SCAN_PRICE_MIN

    combined = final_top_signals + final_top_watch
    
    # ── Terminal Listesi Çıktısı ──
    logging.info("=" * 78)
    logging.info(f"🐂 BOGA AI v117.v4.0 HYBRID — SİNYAL: {len(final_top_signals)} | WATCHLIST: {len(final_top_watch)} | Gevşetme: {relaxation_level}")
    logging.info(f"{'#':<4}{'TICKER':<8}{'SİSTEM':<10}{'BOGA':>6} {'SEKTOR':<22}{'R/R':>5}{'RSI':>6}{'RVOL':>7}")
    logging.info("-" * 78)
    for i, c in enumerate(combined):
        ...
        tip_flag = " ← WATCH" if i >= len(final_top_signals) else ""
        logging.info(
            f"{i+1:<4}{c['ticker']:<8}{sys_name:<10}{boga_s:>6.1f} "
            f"{sector_s:<22}{rr_s:>5.1f}{rsi_s:>6.1f}{rvol_s:>6.2f}x{tip_flag}"
        )
    logging.info("=" * 78)
    ...
    # ── STEP 7: JSON SAVING AND POOL MERGE ──
    now_ny = datetime.now(NY_TZ)
    generated_at = now_ny.isoformat()

    # JSON çıktısı için _pick_json alanını hazırla
    for c in combined:
    ...
    def _wrap_picks(picks_list: list) -> dict:
        ...
        return {
            ...
            "model": "BOGA AI v117.v4.0 (Hybrid Model)",
            "market_regime": MARKET_STATUS.get("regime", "Bull"),
    ...
    try:
        ...
        table_data = []
        for c in final_top_signals:
            if c.get("trigger", {}).get("triggered"):
                z = c.get("boga_zones", {})
                table_data.append({
                    "Date": date_str,
                    "Symbol": c.get("ticker", ""),
    ...
    header = (
        f"🐂 <b>BOGA AI V117.v4.0 — HYBRID SCANNER</b>\n"
        f"🕒 <i>{now_str}</i> | ⏱ {duration:.1f}s | Gevşetme: {relaxation_level}\n"
        f"📊 <i>{len(tickers_to_scan)} tarandı → L1-L2:{len(top_analysis_set)} → "
        f"SİNYAL:{len(final_top_signals)} + WATCH:{len(final_top_watch)}</i>\n"
        f"📈 Market: <b>{MARKET_STATUS['regime']}</b>\n\n"
        "<pre>"
        f"#   SYMBOL  SİSTEM   BOGA   BUY_L   STOP    TP2\n"
        f"──────────────────────────────────────────────────\n"
    )
    rows = []
    for i, c in enumerate(final_top_signals):
        zones = c.get("boga_zones", {})
        ...
        rows.append(
            f"{i+1:02d}. {tag} {c['ticker']:<5} {sys_name:<8} {boga_s:>4.0f}  "
            f"{buy_l:>6.2f} {stop_h:>6.2f} {tp2:>6.2f}"
        )
    for i, c in enumerate(final_top_watch):
        boga_s = c.get("boga_score_100", 0.0)
        rows.append(
    ...
    toplist_msg = header + "\n".join(rows) + "\n──────────────────────────────────────────────────\n</pre>\n"
    toplist_msg += "<i>[INFO] SIGNAL: 15m tetik ONAYLI — WATCH: setup hazır, tetik bekleniyor | BOGA AI v117.v4.0</i>\n\n"
    if final_top_signals:
    ...
    if final_top_watch:
        ...
        for i, c in enumerate(final_top_watch):
            w_lines.append(
                ...
                f"— {c.get('boga_score_100',0):.0f}p | <i>{c.get('selection_system','DEFAULT')}</i>"
            )
        await send_telegram_message(
            "👁️ <b>WATCHLIST — Tetik Bekleyen 10 Aday</b>\n"
            "<i>1D+4H+1H katmanlarını geçti; 15m tetikleyicisi oluşursa sinyale döner.</i>\n\n"
            + "\n".join(w_lines)
        )
    ...
    logging.info(f"[OK] BOGA AI v117.v4.0 Scan complete. ({scanned_count} hisse tarandı | {duration:.1f}s)")


# ================================================================
# ================================================================
# SECTION 17: SCHEDULER (v3.1 MİMARİSİ KORUNDU)
# ================================================================
# ================================================================
...
async def run_scanner():
    await send_telegram_message(
        "🐂 <b>BOGA AI SWING V117.v4.0 — HYBRID MODEL Started!</b>\n"
        "📅 Schedule: Haftaiçi NY 09:00-17:00, her saat başı\n"
        "🎯 FULL_SCAN (09/14/17): 10 Swing Adayı + 10 Watchlist (Dinamik Gevşetmeli)\n"
        "🎯 ENTRY_CHECK (diğer saatler): PENDING adaylar için stratejiye özel tetikleme kontrolü\n"
        f"📊 Market: <b>{MARKET_STATUS.get('regime','Bull')}</b>\n"
        f"🧊 Cooldown: {COOLDOWN_DAYS}g | 📅 Earnings: {EARNINGS_MIN_DAYS}g"
    )
...
if __name__ == "__main__":
    try:
        ...
        if "--oneshot" in sys.argv:
            ...
            print(f"[START] BOGA AI v117.v4.0 HYBRID (One-Shot, mode={oneshot_mode}) baslatildi...")
