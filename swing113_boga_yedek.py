...
# V112: Global Veri Havuzu (Chunk indirmeden gelen verileri saklar)
        ...
        try:
            ...
            for sym in tickers_in_data:
                try:
                    ...
                    # ROC5: 5 günlük getiri (Aşırı düşüşte olmasın, %5 tolerans)
                    roc5 = float(
                    ...
                    # Veriyi Cache'e at (apply_atmaca_filters sonra kullansın)
...
def analyze_smart_money_flow(df_1d: pd.DataFrame, ticker: str, info: dict) -> dict:
    ...
    - VSA (Volume Spread Analysis) - Effort vs Result (Gizli Akümülasyon)
    """
    try:
        if len(df_1d) < 20:
        ...
        # Yüksek Hacim (Çaba) var ama fiyat çok az artmış (Sonuç) ve kapanış mumun zirvesinde.
        recent_3 = df_1d.tail(3)
        avg_vol_20 = volume.rolling(20).mean().iloc[-1]
        
        for idx, row in recent_3.iterrows():
            # Spread: Mumun en düşüğü ile en yükseği arasındaki % fark
            spread_pct = (row['High'] - row['Low']) / row['Close'] if row['Close'] > 0 else 0
            # Göreceli Hacim
            rel_vol = row['Volume'] / avg_vol_20 if avg_vol_20 > 0 else 1.0
            
            # Kapanış Pozisyonu: Fiyatın mum içinde nerede kapandığı (0 = en dip, 1 = en tepe)
            high_low_range = row['High'] - row['Low']
            close_pos = (row['Close'] - row['Low']) / high_low_range if high_low_range > 0 else 0.5
...
# ============================================================
# 🔮 V112 PREDICTIVE OPTIONS (Öncü Yön Tahmini)
# ============================================================
async def analyze_options_sentiment(ticker: str) -> dict:
    """
    KARTAL ALPHA V5.0: Put/Call Oranı ve Opsiyon hacmiyle yön tahmini.
    Fiyat kırılmadan önce akıllı paranın opsiyon tarafında nerede konumlandığını tespit eder.
    """
    try:
        stock = yf.Ticker(ticker)
        exp_dates = await asyncio.to_thread(lambda: stock.options)
        if not exp_dates: return {'bullish': False, 'score': 0.0, 'details': []}

        # En yakın 2 vadeyi al (Sadece yakın vadeli hareketler swing trade için anlamlıdır)
        target_dates = list(exp_dates)[:2]
        total_call_vol = 0
        total_put_vol = 0
        
        for date in target_dates:
            try:
                chain = await asyncio.to_thread(lambda: stock.option_chain(date))
                total_call_vol += chain.calls['volume'].sum() if not chain.calls.empty else 0
                total_put_vol += chain.puts['volume'].sum() if not chain.puts.empty else 0
            except: continue
            
        if total_call_vol == 0 and total_put_vol == 0:
            return {'bullish': False, 'score': 0.0, 'details': []}
            
        # Put / Call Ratio (Hacim bazlı)
        ...
        logging.debug(f"Opsiyon analiz hatası ({ticker}): {e}")
        return {'bullish': False, 'score': 0.0, 'details': []}
...
def detect_rising_stock(df_1d: pd.DataFrame) -> dict:
    ...
    try:
        ...
        if pct_20d > 0 and pct_5d > 0.02:
            ...
            details.append(f"🔄 Pullback Reversal (5G: +%{pct_5d*100:.1f})")
        ...
        if range_10d < 0.06 and pct_2d > 0.015:
            ...
            details.append(f"💥 Base Breakout (Narrow Band → Breakout)")
        ...
        if len(close) >= 4:
            ...
            if chg1 > chg2 > chg3 > 0:
                score += 4.8
                pattern = "Gaining Momentum"
                details.append("🚀 Gaining Momentum (Stronger every day)")
        ...
        if recovery > 0.05 and pct_5d > 0:
            ...
            details.append(f"📈 Bottom Reversal: +%{recovery*100:.1f}")
...
# 1) MARKET REGIME (SPY EMA200 + VIX KONTROLÜ)
    try:
        spy = yf.Ticker("SPY")
        vix = yf.Ticker("^VIX") # VIX Eklendi
        
        hist = spy.history(period="1y")
        vix_hist = vix.history(period="5d")
        
        if not hist.empty and not vix_hist.empty:
            close = hist["Close"]
            ...
            # VIX'i global duruma kaydet (ileride TP/SL daraltmada kullanılabilir)
            MARKET_STATUS["vix"] = current_vix
            
            # MARKET REGIME TESPİTİ (VIX ESNETMELİ 5 SEVİYE)
            if current_price > ema200 and ema200_slope > 0.002 and last_ema50 > ema200 and current_vix < 18:
                MARKET_STATUS["regime"] = "STRONG"
                MARKET_STATUS["min_score_modifier"] = 0.0
                logging.info(f"✅ STRONG MARKET (SPY > EMA50 > EMA200 + Eğim Pozitif | VIX: {current_vix:.1f}). Filtreler Normal.")
                
            elif current_price > ema200 and ema_spread > 0.01 and current_vix < 22:
                MARKET_STATUS["regime"] = "BULLISH"
                MARKET_STATUS["min_score_modifier"] = 0.3
                logging.info(f"⚠️ BULLISH MARKET (SPY > EMA200 | VIX: {current_vix:.1f}). Filtreler Biraz Sıkı.")
                
            elif current_price > ema200 and current_vix < 22:
                MARKET_STATUS["regime"] = "CHOPPY"
                MARKET_STATUS["min_score_modifier"] = 0.5
                logging.info(f"🟡 CHOPPY MARKET (Momentum zayıf | VIX: {current_vix:.1f}). Filtreler Sıkı.")
                
            elif current_vix >= 22 and current_vix < 35:
                # ESNETME BURADA: Piyasa düşüşte veya VIX yüksek olsa bile sistemi tamamen kapatma
                MARKET_STATUS["regime"] = "HIGH_VOLATILITY"
                MARKET_STATUS["min_score_modifier"] = 0.5 # Aday bulabilmesi için barajı makul tutuyoruz
                logging.info(f"⚡ HIGH VOLATILITY (VIX: {current_vix:.1f}). Vur-Kaç / Fırsat Modu Aktif.")
                
            else:
                MARKET_STATUS["regime"] = "WEAK"
                MARKET_STATUS["min_score_modifier"] = 1.0
                logging.warning(f"🐻 WEAK/CRASH MARKET (VIX: {current_vix:.1f} | SPY < EMA200). Screening DURDURULACAK.")
...
def get_stock_data(
    ...
    """
    KARTAL V5.0 OPTİMİZE: Sıfır Network I/O (1D için) ve Hızlı Fetch (1H için).
    Universe builder'dan gelen BULK_DATA_CACHE'i ana kaynak olarak kullanır.
    """
    t = ticker.strip().upper()

    # 1. ÖNCELİK: 1D VERİSİ İÇİN TAMAMEN BELLEK (NETWORK YASAK)
    if interval == "1d":
        if t in BULK_DATA_CACHE:
            # Bellekteki veriyi kopyalayarak döndür (Orijinal cache bozulmasın)
            return BULK_DATA_CACHE[t].copy()
        else:
            # Eğer 1D verisi bulk cache'de yoksa, bu hisse evren dışıdır.
            # Tekrar internete gidip vakit kaybetme ve ban riski alma.
            return None

    # 2. ÖNCELİK: 1H VERİSİ İÇİN HIZLI FETCH
    # 1H verileri toplu indirilmediği için mecburen çekilecek, 
    # ancak bekleme süreleri (sleep) agresif şekilde düşürüldü.
    time.sleep(random.uniform(0.1, 0.3)) 
    
    try:
        stock = yf.Ticker(t)
        # 1H için sadece son 7 gün yeterli (V112 Standardı)
        df = stock.history(period="7d", interval="1h", auto_adjust=True, timeout=10)

        if df is None or df.empty:
            return None

        # Sütun formatlama ve temizlik
        ...
        if "Too Many Requests" in str(e):
            logging.error(f"🚨 RATE LIMIT! Yahoo 1H fetch sırasında durdurdu ({t}).")
        else:
            logging.error(f"❌ {t} (1h) fetch hatası: {e}")
        return None
        
# ============================================
# 2) YF INFO ÇEKİCİ — MARKET CAP, VOLUME, BETA, SHORT FLOAT (V112 OFFLINE)
# ============================================

def get_stock_info(ticker: str) -> dict:
    """
    KARTAL V5.0 OPTİMİZE: YF info() çağrıları tamamen kaldırıldı! (Sıfır Network I/O)
    Ban riskini önlemek için sadece önceden diske yazılan persistent_info_cache kullanılır.
    """
    t = ticker.strip().upper()
    
    global persistent_info_cache
    
    # Sadece diske kaydedilen kalıcı bellekten oku, asla internete gitme
    if t in persistent_info_cache:
        return persistent_info_cache[t]
    
    # Eğer cache'de yoksa, botu yavaşlatmamak ve ban yememek için güvenli boş değerler dön.
    return {
        "market_cap": 0,
        "avg_volume": 0,
        "beta": 1.0,
...
# 4. VOLUME ANALYSIS (KURUMSAL PARA AKIŞI - MICRO MOMENTUM ODAKLI)
def analyze_volume_profile_flow(df_1d: pd.DataFrame, ticker: str) -> dict:
    """Kurumsal para giriş/çıkışını ve erken swing hacim akışını analiz eder. (Kartal V5.0 - Micro Momentum)"""
    try:
        # Hantallığı gidermek için 5 gün yerine son 2 günlük (Micro-RVOL) hacme bakıyoruz.
        vol_20d = df_1d['Volume'].iloc[-20:].mean()
        vol_2d = df_1d['Volume'].iloc[-2:].mean()
        vol_ratio = vol_2d / vol_20d if vol_20d > 0 else 1.0
        
        # Accumulation / Distribution (yeşil vs kırmızı hacim) - Son 3 güne odaklan (Daha çevik)
        recent = df_1d.iloc[-3:].copy()
        ...
        # 2 günlük hacim hafif artmış ve alıcılar net baskınsa
        if 1.2 <= vol_ratio <= 1.5 and accum_score > 1.2:
            bonus = 1.2
            msg = f"🐋 Micro-Akümülasyon: Sessiz Para Girişi (+%{(vol_ratio-1)*100:.0f})"
            has_flow = True

        # 2️⃣ GÜÇLÜ MOMENTUM AKIŞI (uyanış) 
        # Son 2 günde %50'den fazla ekstra hacim ve çok güçlü alıcı varsa
        elif vol_ratio > 1.5 and accum_score > 1.4:
            bonus = 1.6
            msg = f"💰 Micro-Momentum: Ani Hacim Patlaması (+%{(vol_ratio-1)*100:.0f})"
            has_flow = True
...
async def apply_atmaca_filters(ticker: str) -> dict | None:
    ...
    try:
        ...
        # ========================================
        # VERİ TOPLAMA (V112 - SIFIR NETWORK I/O STRATEJİSİ)
        # ========================================
        # DİKKAT: yf.Ticker(ticker) çağrısı ve stock.info/fast_info tamamen kaldırıldı.
        # Bu sayede her hisse için beklenen 2-4 saniyelik gecikme 0 ms'ye iner.
        
        global persistent_info_cache
        current_time = time.time()
        
        # 1. ÖNCELİK: Persistent Cache (Kalıcı Bellek)
        # Market Cap verisini önce yerel dosyadan çekiyoruz.
        cached_data = persistent_info_cache.get(ticker, {})
        market_cap = cached_data.get("market_cap", 0)
        ts = cached_data.get("ts", 0)

        # 2. ÖNCELİK: Eğer cache boşsa veya 15 günden eskiyse BULK_DATA_CACHE'den kurtarmayı dene
        # Not: yf.download bazen mcap döndürmez, bu yüzden 0 ise yf.Ticker çağırmak yerine 
        # süreci bozmamak adına 0 kabul edip devam ediyoruz (Ban riskinden daha güvenli).
        if market_cap == 0 or (current_time - ts > 1296000):
            # Market Cap bilgisi kritik bir 'Hard Filter' değilse yf.Ticker çağırmıyoruz.
            # Sadece mevcut cache verisini koru (0 olsa bile network'e gitme).
            pass

        # Sektör ve Beta bilgilerini de yerel fonksiyondan (cache destekli) al
...
# ========================================
        # 🥈 KATMAN 2: AKIŞ & MOMENTUM FİLTRESİ (Dinamik - Micro Momentum)
        # RVOL = Son 2 Günlük Ortalama Hacim / Son 20 Günlük Ortalama Hacim (Hantallık Giderildi)
        # Şartlar: 1D Close > EMA50, EMA20 > EMA50, ADX > 12,
        #          Son 10 günde min 5 yeşil mum, CMF > 0
        # ========================================
        layer2_pass = True
        layer2_reasons = []

        # YENİ RVOL HESABI (Micro-Momentum: Son 2 gün vs Son 20 Gün)
        try:
            vol_2g_avg = float(volume_1d.tail(2).mean()) if len(volume_1d) >= 2 else 0.0
        ...
        # Geriye dönük uyumluluk (Aşağıdaki kodlar ve Katman 3 formülü kırılmasın diye değişkeni eşitliyoruz)
        rvol_5_30 = rvol_micro 

        # Eşik değeri 1.30'dan 1.15'e çekildi (Piyasa hacmine uyum için esnetildi)
        if rvol_micro < 1.15:
            layer2_pass = False
            layer2_reasons.append(f"Micro-RVOL(2g/20g)={rvol_micro:.2f} < 1.15")
        ...
        try:
            ...
            if not np.isnan(ema20_val) and not np.isnan(ema50_val):
                ...
                if not (current_price > ema50_val and ema20_val > ema50_val):
                    layer2_pass = False
                    reason = "P < EMA50" if current_price <= ema50_val else "EMA20 < EMA50"
                    layer2_reasons.append(f"Trend kırık ({reason} | P:{current_price:.2f} EMA20:{ema20_val:.2f} EMA50:{ema50_val:.2f})")
        except:
        ...
        # Son 10 günde en az 3 yeşil mum
        try:
        ...
        if green_candles < 3:
            layer2_pass = False
            layer2_reasons.append(f"Yeşil mum={green_candles} < 3 (son 10 gün)")
        ...
        if bb_squeeze and ema_stack and rvol_today > 1.3:
            ...
            score += 4.8
            entry_trigger = "Trend Continuation Swing"
...
def build_candidate_block(rank: int, c: dict) -> str:
    ...
    # 🔮 V113 EKLENTİSİ: Opsiyon Öncü Sinyali Verisini Çek
    opt_data = c.get("opt_sentiment", {})
    opt_pcr = opt_data.get("pcr", "N/A")
    opt_msg = next((d.split(": ")[-1] for d in details_list if "Opsiyon Sinyali" in d), "➖ Nötr / Sinyal Yok")

    momentum_block = (
        "📌 <b>Momentum & Giriş</b>\n"
        f"• OBV Eğilim: {'📈 Yükseliş' if obv_up else '➖ Nötr/Zayıf'}\n"
        f"• Hacim: {vol_info}\n"
        f"• Opsiyon (PCR {opt_pcr}): {opt_msg}\n"
    ...
    return (
        ...
        f"📈 Expected Profit: %{pct:.1f} | R/R: {rr:.2f}x | Score: {score:.1f}\n"
        f"⏳ Holding: {hold} days\n"
...
async def scan_top_stocks():
    ...
    # ============================================================
    # 🥉 KATMAN 3: AĞIR VERİLER (V112 OFFLINE INFO & SMART FETCH)
    # ============================================================
    logging.info(f"🏦 Katman 3 Başlıyor: Top {len(top_40)} aday için derin analiz (Insider, Opsiyon, Risk) tetikleniyor...")
    
    async def fetch_heavy_data_for_candidate(c: dict):
        ticker = c["ticker"]
        try:
            # ÖNEMLİ: stock.info ÇAĞRISI KALDIRILDI. Veriyi cache fonksiyonundan çekiyoruz.
            info = get_stock_info(ticker) 
            
            # Sektör ve Beta verisini cache'den güncelle
            c["sector"] = info.get("sector", c.get("sector", "Unknown"))
            c["beta"] = info.get("beta", c.get("beta", 1.0))

            # Ağır veri çekimi için Ticker objesi (Insider ve Opsiyonlar için gerekli)
            stock = yf.Ticker(ticker)

            # 👔 INSIDER İŞLEM TESPİTİ (Sadece Top 40 için izin verilen network yükü)
            ...
            # 📊 FİNANSAL SAĞLIK ANALİZİ (Cache verisi üzerinden çalışır)
            ...
            # 🎯 KATALİZÖR TESPİTİ (Cache verisi üzerinden çalışır)
            catalyst_result = check_silent_catalysts(ticker, info)
            ...
            if c.get('score', 0) > 20.0:
                ...
                if risk_res.get('has_risk'):
                    ...
                    logging.warning(f"🚨 {ticker} Dava Riski: Puan kırıldı.")

            # 🔮 V112 PREDICTIVE OPTIONS (Opsiyon Akış Tahmini)
            opt_sentiment = await analyze_options_sentiment(ticker)
            if opt_sentiment.get('bullish') or opt_sentiment.get('pcr', 1.0) > 1.2:
                c["score"] += opt_sentiment.get('score', 0)
                c["details"].extend(opt_sentiment.get('details', []))
                c["opt_sentiment"] = opt_sentiment
                
        except Exception as e:
            logging.debug(f"⚠️ {ticker} Katman 3 hatası: {e}")

    # Eşzamanlı maksimum 3 istek (Rate limit koruması için kritik)
    sem_k3 = asyncio.Semaphore(3)
    async def sem_fetch_heavy(c):
        async with sem_k3:
            # Network yükünü dağıtmak için küçük bir jitter
            await asyncio.sleep(random.uniform(0.1, 0.4))
            await fetch_heavy_data_for_candidate(c)

    # Top 40 için paralel işlemi başlat
    await asyncio.gather(*(sem_fetch_heavy(c) for c in top_40))

    # Nihai Skor Sıralaması (Yeni verilerle)
    top_40.sort(key=lambda x: x.get("score", 0.0), reverse=True)
    
    # Diversified Top 20 Seçimi
    top_candidates = build_diversified_toplist(top_40, total=20)
    logging.info(f"🎯 Katman 3 Tamamlandı: {len(top_candidates)} nihai aday hazır.")
    
    # ============================================================
    # 🦁 ALPHA VANTAGE FİYAT DOĞRULAMASI (KARTAL V5.0 - HIZLANDIRILMIŞ)
    # ============================================================
    if ENABLE_ALPHA_VALIDATION:
        # Sadece gerçekten en yüksek potansiyelli (Skor >= 35) olanları ayır
        # Bu sayede 20 hisse yerine sadece en kritik 3-5 hisse için 12sn bekleriz.
        high_conviction = [c for c in top_candidates if c.get('score', 0) >= 35.0]
        
        if high_conviction:
            logging.info(f"🦁 En yüksek skorlu {len(high_conviction[:5])} aday için AV doğrulaması başlıyor...")
            
            # Ücretsiz API limiti nedeniyle sadece ilk 5 aday ile sınırlandırıyoruz
            for idx, c in enumerate(high_conviction[:5]):
                ...
                if not av_result.get('validated', True) and av_result.get('av_price', 0) > 0:
                    c['score'] -= 10.0 
                    logging.warning(f"⛔ {c['ticker']} elendi: Fiyat farkı %{av_result.get('price_diff_pct', 0):.1f}")
                
                # Eğer sırada başka aday varsa ve limit dolmadıysa bekle
                if idx < len(high_conviction[:5]) - 1:
                    await asyncio.sleep(12)
        else:
            logging.info("🦁 AV Doğrulaması: 35.0 puan barajını geçen aday yok, zaman kazanmak için atlanıyor.")
    ...
    try:
        ...
        def build_pick_entry(i: int, c: dict) -> dict:
            ...
            return {
                ...
                "reasoning": f"Atmaca V112 Model: {c.get('entry_trigger','SWING')} signal detected. High breakout potential with a confidence score of {c['score']:.1f}.",
                "pattern": c.get("entry_trigger", "SWING"),
