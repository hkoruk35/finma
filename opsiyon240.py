"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   🚀 BOGA AI v240 — SEKTÖR ÖNCE + GERÇEK BACKTESTING                      ║
║   "Sektör → Lider → Sıkışma → Hacim → Flow → Doğru Kontrat"               ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
║  v230'dan farkı — 5 KRİTİK DEĞİŞİKLİK:                                    ║
║                                                                              ║
║  1. SEKTÖR ÖNCE pipeline  ← en büyük değişiklik                            ║
║     v230: 500 hisse → sektör skor olarak ekleniyor                         ║
║     v240: ETF taraması önce (6 ETF, 10 saniye) →                           ║
...
║     v230: gamma_eff log-capped (tek boyutlu)                               ║
║     v240: gamma / (ask × |theta_günlük|)                                   ║
...
║     v230: earnings yakınsa -5 puan                                         ║
║     v240: earnings < 14 gün = tamamen listeden çıkar                       ║
...
║                                                                              ║
║  5. BACKTESTING TAMAMLANDI  ← gerçek sonuç takibi                          ║
║     v230: peak_pct ve hit_40pct kaydediliyor ama dolduran kod yok          ║
║     v240: fill_backtest_results() her tarama sonrası çalışır               ║
...
# ── DTE: günlük swing odaklı 7-55 gün ───────────────────────────────────
DTE_MIN  = 7           # NR7 + güçlü sıkışma için 7g alabiliriz
DTE_MAX  = 55          # Sweet spot üst sınır
DTE_HARD_MIN = 7       # Asla bu altına inme

# ── Earnings hard block (YENİ) ───────────────────────────────────────────
EARNINGS_HARD_BLOCK_DAYS = 14   # Bu kadar yakın earnings = tamamen çıkar
EARNINGS_WARN_DAYS       = 21   # Bu kadar yakın = ağır ceza
...
# Gamma/Theta oran eşiği (YENİ) — bu altı verimsiz kontrat
GAMMA_THETA_RATIO_MIN = 0.5   # gamma / (ask × |theta_günlük|)
...
def get_preferred_dte(setup_type: str, squeeze_bonus: float,
                      flow_bonus: float, vol_bonus: float,
                      entry_mode: str) -> Tuple[int, int]:
    """
    *** DEĞİŞİKLİK 4: Setup tipine göre DTE seçimi ***

    NR7 + kritik sıkışma + güçlü RVOL → 7-14g  (hızlı gamma)
    EMA breakout / golden cross          → 14-21g (biraz daha süre)
    Trend pullback / EMA50 sekme         → 21-35g (tempo setup)
    Steady / belirsiz                    → 35-55g (zaman satın al)
    """
    is_explosive = setup_type in ("NR7_KRITIK", "NR7") and vol_bonus >= 3
    is_breakout  = "EMA200_BREAKOUT" in entry_mode or "GOLDEN_CROSS" in entry_mode

    if is_explosive and flow_bonus >= 3:
        return (7, 14)       # Gamma squeeze — kısa vade
    elif is_explosive or is_breakout:
        return (14, 21)      # Güçlü setup — orta kısa
    elif "EMA50_PULLBACK" in entry_mode or "TREND_BIRTH" in entry_mode:
        return (21, 35)      # Trend setup — orta
    elif squeeze_bonus >= 6 or flow_bonus >= 5:
        return (21, 35)      # Güçlü sinyal ama setup yavaş
    else:
        return (35, 55)      # Varsayılan sweet spot
...
async def stage9_contract_selection(ticker: str, cp: float, close: pd.Series,
    ...
    try:
        ...
        pref_min, pref_max = get_preferred_dte(
            setup_type   = s5.get("setup_type", "HAFIF"),
            squeeze_bonus= s5.get("squeeze_bonus", 0),
            flow_bonus   = s8.get("flow_bonus", 0),
            vol_bonus    = s7.get("vol_bonus", 0),
            entry_mode   = ema.get("entry_mode", ""),
        )
        ...
        for exp_str in exps:
            ...
            for _, row in calls.iterrows():
                try:
                    ...
                    if oi < 5 or mid < MID_MIN: continue
                    ...
                    # *** DEĞİŞİKLİK 2: Gamma/Theta oranı hesapla ***
                    gt_ratio = calc_gamma_theta_ratio(gamma, ask, theta)

                    # ── GAMMA SWEET SPOT (0.10-0.60 delta) ─────────────────
                    if DELTA_GAMMA_MIN <= delta <= DELTA_GAMMA_MAX:
                        ...
                        # DTE ivme katsayısı (günlük swing: kısa DTE = daha yüksek)
                        if dte <= 10:    accel = 3.0
                        elif dte <= 14:  accel = 2.5
                        elif dte <= 21:  accel = 2.0
                        elif dte <= 35:  accel = 1.5
                        else:            accel = 1.2

                        cw_pen    = -5.0 if cw_danger and strike > cp * 0.99 else 0.0
                        vol_boost = min(s7.get("vol_bonus", 0) * 0.3, 3.0)

                        # *** Gamma/Theta oranı puanlamaya dahil (%30 ağırlık) ***
                        gt_score = 0.0
                        if gt_ratio >= 2.0:   gt_score = 6.0   # Çok verimli
                        elif gt_ratio >= 1.0: gt_score = 4.0   # Verimli
                        elif gt_ratio >= 0.5: gt_score = 2.0   # Kabul edilebilir
                        else:                 gt_score = 0.0   # Verimsiz

                        sc = (fs + geff * 50.0 + delta * 3.0 + liq +
                              accel * 2.0 + dte_bonus + cw_pen + vol_boost + gt_score)

                        if sc > gamma_top:
                            gamma_top = sc
                            atr_move  = min(max(l4.get("atr_pct", 2.0) / 100 * 4.0, 0.04), 0.25)
                            gamma_best = {
                                ...
                                "gt_ratio": round(gt_ratio, 3),   # YENİ
                                "gt_score": round(gt_score, 1),   # YENİ
                    ...
                    # ── KURUMSAL (0.40-0.75 delta) ─────────────────────────
                        ...
                        if dte <= 14:    accel_k = 2.0
                        elif dte <= 21:  accel_k = 1.5
                        elif dte <= 35:  accel_k = 1.2
                        else:            accel_k = 1.0

                        # Gamma/Theta kurumsal için de dahil
                        gt_score_k = min(gt_ratio * 2.0, 4.0)

                        sc = delta * 4.0 + gamma * 1000.0 + liq + sweep + dte_bonus + accel_k + gt_score_k

                        if sc > safe_top:
                            safe_top = sc
                            atr_move = min(max(l4.get("atr_pct", 2.0) / 100 * 3.0, 0.03), 0.18)
                            safe_best = {
                                ...
                                "gamma_accel": accel_k,
                                "gt_ratio": round(gt_ratio, 3),   # YENİ
                                "gt_score": round(gt_score_k, 1), # YENİ
                                "call_wall_danger": cw_danger,
                                "cost_per_contract": round(ask * 100, 0),
                                "score": round(sc, 2),
                                "sim": bs_pnl_sim(cp, strike, iv_row, dte, atr_move, sim_days),
    ...
    for rec in pending:
        try:
            ...
            for i, (hi, lo, cl) in enumerate(zip(
                ...
                pct_hi = (hi - entry_price) / entry_price * 100
                pct_lo = (lo - entry_price) / entry_price * 100
                # Hisse bazlı proxy: opsiyonun %40 kazanması için hissenin
                # ATR × 2 hareket etmesi gerekiyor (kaba tahmin)
                if pct_hi >= 8.0:    # ~%40 opsiyon kazancı proxy
                    hit_40pct = True
                if pct_lo <= -4.0:   # ~%30 opsiyon kaybı proxy (stop)
                    hit_sl = True
...
def build_block(c: dict) -> str:
    ...
    for key in ["gamma_sweet", "institutional"]:
        ...
        lines.append(
            f"  🎯 TP:${od['tp_price']:.2f}  SL:${od['sl_price']:.2f}  "
            f"Zaman:{od['time_stop_days']}g  Decay:%{od.get('daily_decay_pct', 0):.1f}/g"
        )
    return "\n".join(lines)
...
def build_report(candidates, vix, duration, n_scanned, s0: dict,
    ...
    summary = (
        f"🚀 <b>BOGA AI v240 — SEKTÖR ÖNCE + GERÇEK BT</b>\n"
...
def save_picks(candidates, n_universe, duration, active_sectors):
    try:
        out  = os.path.join(DATA_DIR, f"v240_{datetime.now().strftime('%Y%m%d_%H%M')}.json")
        data = {
            "version": "v240",
...
async def scan():
    ...
    await send_tg(
        f"✅ <b>v240 Tamamlandı!</b>  {duration:.0f}sn  "
...
if __name__ == "__main__":
    ...
    if "--oneshot" in sys.argv:
        ...
        # --eod parametresi ile: python boga_ai_v240.py --oneshot --eod
        SCAN_SESSION = "eod" if "--eod" in sys.argv else "morning"
        print(f"🚀 BOGA AI v240 (One-Shot) — session: {SCAN_SESSION}")
