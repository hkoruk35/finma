...
def main():
    ...
    subprocess.run([VENV_PYTHON, script_path, "--force"], cwd=FINMA_DIR)
    ...
    if os.path.exists(refresh_path):
        subprocess.run([VENV_PYTHON, refresh_path], cwd=FINMA_DIR)

    # 3. Git Push (Silent update)
    try:
        subprocess.run(["git", "add", "."], cwd=FINMA_DIR)
        subprocess.run(["git", "commit", "-m", f"Data: Terminal Pulse {now_ny.strftime('%H:00')}"], cwd=FINMA_DIR)
