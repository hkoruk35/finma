"""
🐂 BOGA AI v6.1 — Swing/Position Opportunity Scanner
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
def build_summary_report(candidates: List[dict], vix: float, duration: float, universe_size: int) -> Tuple[str, str]:
    ...
    header = (
        f"🐂 <b>BOGA AI v6.1 (Opportunity Scanner)</b>\n"
        f"📅 <i>{now_str}</i>\n"
...
# ════════════════════════════════════════════════════════════════════════════
# 9.5) JSON EXPORT — Web sitesi için options_picks.json üret
# ════════════════════════════════════════════════════════════════════════════

def _opt_to_json(o: dict) -> dict:
    """Tek bir opsiyon sözlüğünü JSON-safe forma çevir."""
    if not o:
        return None
    return {
        "strike":            o.get("strike"),
        ...
        "oi":                o.get("oi"),
        "volume":            o.get("volume"),
        "vol_oi_ratio":      round(o.get("volume", 0) / o.get("oi", 1), 3) if o.get("oi") else None,
        "contract_cost":     o.get("cost_per_contract"),
        "breakeven":         o.get("breakeven"),
        "tp_price":          o.get("tp_price"),
        "sl_price":          o.get("sl_price"),
        "time_stop_days":    o.get("time_stop_days"),
        "daily_decay_pct":   o.get("daily_decay_pct"),
        "theta_delta_ratio": o.get("theta_delta_ratio"),
        "sim_gain_pct":      o.get("sim", {}).get("pnl_pct") if isinstance(o.get("sim"), dict) else None,
        "liq_score":         o.get("liq_score"),
        "sweep_score":       o.get("sweep_score"),
    }

def build_options_json(candidates: list, vix: float, duration: float, universe_size: int) -> dict:
    now_dt = datetime.now(NY_TZ)
    mode_labels = {
        "EMA200_BREAKOUT":          "EMA200 Breakout",
        "EMA200_BREAKOUT_BELOW200": "EMA200 Breakout (Below 200)",
        "GOLDEN_CROSS":             "Golden Cross",
        "GOLDEN_CROSS_BELOW200":    "Golden Cross (Below 200)",
        "NEAR_GOLDEN":              "Near Golden Cross",
        "TREND_BIRTH":              "Trend Birth",
        "ESTABLISHED_TREND":        "Established Trend",
        "EMA50_BOUNCE":             "EMA50 Bounce",
    }
    regime_counts = {"trend": 0, "breakout": 0, "neutral": 0}
    for c in candidates:
        r = c.get("options", {}).get("regime", "neutral")
        regime_counts[r] = regime_counts.get(r, 0) + 1
    ...
    for i, c in enumerate(candidates):
        ...
        picks.append({
            ...
            "grade":         c.get("grade", ""),
            "entry_mode":    entry_mode,
            "entry_mode_label": mode_labels.get(entry_mode, entry_mode),
            ...
            "hv30":          c.get("hv30"),
            "iv_pct":        opt.get("atm_iv"),
            "iv_rank":       opt.get("iv_rank"),
            "iv_vs_hv":      opt.get("iv_vs_hv"),
            "vwap":          l2.get("vwap"),
            "vwap_ok":       l2.get("vwap_ok", True),
            ...
            "dte":           opt.get("dte"),
            "institutional": _opt_to_json(opt.get("institutional")),
            "asymmetric":    _opt_to_json(opt.get("asymmetric")),
            "uoa_score":     uoa.get("uoa_score", 0),
            "uoa_signal":    uoa.get("uoa_signal", "—"),
            "earnings_warning": uoa.get("earnings_warning", False),
            "golden_cross":  l2.get("golden_cross", False),
            "ema200_breakout": l2.get("ema200_breakout", False),
            "breakout_base_score": c.get("breakout_base_score"),
        })
    ...
    return {
        ...
        "vix":            round(vix, 1),
        "vix_regime":     MARKET_VIX.get("regime", ""),
        "spy_return_60d": round(SPY_RETURN_CACHE.get("return_60d", 0), 1),
...
def save_options_json(data: dict):
    """transfer/latest/ ve data/{date}/ klasörlerine options_picks.json yaz."""
    here    = os.path.dirname(os.path.abspath(__file__))
    date_str = data["date"]

    targets = [
        os.path.join(here, "transfer", "latest"),
        os.path.join(here, "data", date_str),
    ]
    for folder in targets:
        os.makedirs(folder, exist_ok=True)
        fp = os.path.join(folder, "options_picks.json")
        with open(fp, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
        logging.info(f"✅ options_picks.json kaydedildi: {fp}")
...
async def scan():
    ...
    # ── P&L Tracker (pozisyonları güncelle) ────────────────────────────────
    try:
        import importlib.util, sys as _sys
        tracker_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "options_pnl_tracker.py")
        if os.path.isfile(tracker_path):
            spec = importlib.util.spec_from_file_location("options_pnl_tracker", tracker_path)
            mod  = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(mod)
            await mod.run_tracker()
            logging.info("✅ P&L tracker tamamlandı")
    except Exception as e:
        logging.error(f"P&L tracker hatası: {e}")
