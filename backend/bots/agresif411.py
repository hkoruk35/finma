...
# ================================================================
# 🚀 ATMACA AGRESİF VURKAÇ MODELİ V411 — YÜKSEK MOMENTUM & HACİM AVCI
# (swing112'nin devasa evren taraması üzerine inşa edildi)
# ================================================================
# V411 YENİLİKLER (Kripto, Biyoteknoloji ve Ani Patlama Hisseleri İçin):
# ✅ 🥇 KATMAN 1 — Vektörel Hacim Radarı (Günlük güncelleme):
#    — Fiyat: $5–$1000 | Market Cap > 300M | 30g Avg DV > 5M USD
#    — Erken RVOL: Son 2 Gün Ortalaması / 30g Ort > 1.2 (Ani hacim patlaması)
# ✅ 🥈 KATMAN 2 — Hyper-Momentum Filtresi (Dinamik Hard Filter):
#    — RVOL (Taze Para): BUGÜNÜN (Anlık) Hacmi / 30g Ort > 1.50 ZORUNLU
#    — 1D Close > EMA20 yeterli (Kırılımları kaçırmamak için EMA50 zinciri kırıldı)
#    — ADX > 18 zorunlu
#    — Yorulma Tavanı Genişletildi: RSI > 85 ve 3G ROC > %25'e kadar izin verildi
#    — CMF > 0 (Para girişi onayı)
# ✅ 🥉 KATMAN 3 — Composite Ranking Motoru:
#    — RVOL_Bugün_zscore*0.40 + Trend*0.25 + 5g_Accel*0.20
#      + ADX*0.05 + DollarVol*0.05 + VolExpand*0.05
#    — En yüksek momentumlu ilk 200 hisse seçilir
# ✅ UNIVERSE TTL: 168 saat → 24 saat (Her gün güncellenir)
# ================================================================

# 🔹 Log ve Uyarı Ayarları (v411 Optimized)
logging.basicConfig(
    level=logging.INFO,
...
DAILY_RUN_HOUR = 14          # Her gün NY saati ile 14:00
DAILY_RUN_MINUTE = 0
...
# --- V411 SWING TRADE ODAKLI AYARLAR ---
MIN_RR_RATIO = 1.2
...
async def build_atmaca_universe_full() -> List[str]:
    ...
    YENİ STRATEJİ (V411 Agresif Vurkaç):
    ─────────────────────────────────────────────────────────
    ...
    AŞAMA 2 — VEKTÖREL FİLTRELEME  (Pandas, anlık)
      • Fiyat $5–$1000
      • Ortalama Hacim ≥ ATMACA_MIN_AVG_VOLUME
      • Dollar Volume ≥ ATMACA_MIN_DOLLAR_VOLUME
      • RVOL = son2g_ort / son30g_ort ≥ 1.2   (Ani hacim patlaması zorunlu)
        ...
        try:
            ...
            for sym in tickers_in_data:
                try:
                    ...
                    # VURKAÇ RVOL: Sadece son 2 günün hacmi patlamış mı ona bak!
                    avg_vol_2 = float(volume.tail(2).mean())
                    rvol = (avg_vol_2 / avg_vol_30) if avg_vol_30 > 0 else 0.0
                    if rvol < 1.2: # Agresif filtre: Hacim aniden en az %20 artmış olmalı
                        continue
...
# ============================================================
# 🦅 ATMACA 8-FACTOR BALANCED RANKING ENGINE V411 (Profit-Focused)
# ============================================================
def compute_multi_factor_score(c: dict) -> float:
    """
    🥉 KATMAN 3 — ATMACA RANKING MOTORU V411
...
# ======================================================
#  NY SAATİ 14:00 ZAMAN KONTROLÜ
# ======================================================

def is_time_to_run() -> bool:
    """Her gün sadece NY saatiyle 14:00'de çalışır."""
...
# ============================================================
# V411 YENİ MODÜLLER: Black-Scholes, Smart Money, Insider, Finansal Sağlık
# ============================================================
...
async def scan_option_chain(ticker: str, current_price: float, profit_target: float = 0.0) -> dict:
    """
    🦅 V411 OPSİYON RADARI: Greeks, Breakeven, ITM/OTM analizi (Agresiften alınan + geliştirilmiş)
    """
    try:
...
# ============================================================
# 🏦 V411 INSIDER İŞLEM TESPİT MOTORU (YENİ)
# ============================================================
...
# ============================================================
# 💰 V411 GELİŞMİŞ KURUMSAL PARA AKIŞI (Smart Money Flow)
# ============================================================

def analyze_smart_money_flow(df_1d: pd.DataFrame, ticker: str, info: dict) -> dict:
    """
    V411: Gelişmiş kurumsal para akışı analizi.
...
# ============================================================
# 📊 V411 FİNANSAL SAĞLIK ANALİZİ (Ticari Yaklaşım)
# ============================================================

def analyze_financial_health(ticker: str, info: dict) -> dict:
    """
    V411: Akademik değil TİCARİ yaklaşımla şirket finansal sağlık analizi.
...
# ============================================================
# 📈 V411 YÜKSELEN HİSSE TESPİT MOTORU (Rising Stocks)
# ============================================================

def detect_rising_stock(df_1d: pd.DataFrame) -> dict:
    """
    V411 YENİ: Sadece en güçlüleri değil, yükselmeye BAŞLAYAN hisseleri tespit eder.
...
# ============================================
# 1) YF VERİ ÇEKİCİ — 1D / 1H (V411 - Agresiften alınan)
# ============================================
...
def get_stock_data(
    ...
    V411: 4H yerine 1H kullanılıyor (agresif bottan alınan).
    """
...
def calculate_profit_target(
    ...
    """
    V411 — ATR-Based Dinamik Tavan Sistemi (Precision TP/SL)
    ...
    # ---------------------------------------------------
    # 2) ATR tabanlı TP (V411: 1.8x–2.5x bant)
    ...
    # ---------------------------------------------------
    # 3) Kâr Yüzdesi Hesabı ve %10–12 Tavan (V411 Tavan Sistemi)
    # ---------------------------------------------------
...
def estimate_hold_time(
    ...
    """
    V411 — Piyasa Direnci Hesabı (Directional Efficiency)
    ...
    # ---------------------------------------------------------
    # 5) Kesin Sınırlar: min 3, max 15 (V411 kuralı)
    # ---------------------------------------------------------
...
# 2. KATALİZÖR TESPİT MOTORU (V411 GELİŞTİRİLMİŞ)
def check_silent_catalysts(ticker: str, info: dict) -> dict:
    """V411: Insider buying, Short Squeeze, Analyst Upgrade, Buyback tespiti."""
    catalysts = []
    score = 0.0
...
async def apply_atmaca_filters(ticker: str) -> dict | None:
    ...
    try:
        ...
        # VURKAÇ RVOL (Sadece Bugünün Hacim Patlaması - Taze Para)
        try:
            vol_today = float(volume_1d.iloc[-1])
            vol_30g_avg = float(volume_1d.tail(30).mean()) if len(volume_1d) >= 30 else vol_today
            rvol_today_raw = (vol_today / vol_30g_avg) if vol_30g_avg > 0 else 0.0
        except:
            rvol_today_raw = 0.0

        if rvol_today_raw < 1.50: # Sadece BUGÜN normalin 1.5 katı hacim yapan roketleri al!
            layer2_pass = False
            layer2_reasons.append(f"RVOL(Today)={rvol_today_raw:.2f} < 1.50")

        # 1D Trend Esnekliği: Fiyatın EMA20 üzerinde olması kafidir. (Kırılımları kaçırmamak için EMA50 zinciri kırıldı)
        try:
            ema20_val = float(ema20_1d.iloc[-1])
            if not np.isnan(ema20_val):
                if not (current_price > ema20_val):
                    layer2_pass = False
                    layer2_reasons.append(f"Fiyat EMA20 Altında (P:{current_price:.2f} EMA20:{ema20_val:.2f})")
        except:
            pass

        # ADX > 18 (V411 Agresif Momentum Kuralı)
        if adx_1d > 0 and adx_1d < 18:
            layer2_pass = False
            layer2_reasons.append(f"ADX={adx_1d:.1f} < 18 (Yetersiz Momentum)")
        ...
        score += 1.5
        details.append(f"✅ KATMAN 2: Akış & Momentum Onaylı (RVOL:{rvol_today_raw:.2f}x | Yeşil:{green_candles}/10 | CMF:{cmf_val:.3f})")
        if rvol_today_raw >= 2.0:
            score += 0.5
            details.append(f"🔥 RVOL(Today) Hyper-Agresif: {rvol_today_raw:.2f}x")
        ...
        # ========================================
        # V411 YORULMA SENSÖRÜ (Exhaustion Check)
        # ========================================
        is_exhausted = False
        ...
        try:
            ...
            # VURKAÇ Yorulma Koşulları (Sınırlar agresif momentum için genişletildi):
            # 1) Son 3 günde %25+ yükseliş (gerçek parabolik)
            # 2) RSI > 85 (Momentum hisseleri 75-85 arası asıl patlamayı yapar)
            exhaustion_by_roc = roc_3d > 25.0
            exhaustion_by_rsi = rsi_1d_val > 85.0
        ...
        # ========================================
        # 1H MİKRO / TETİK ANALİZİ (V411: 4H yerine 1H)
        # ========================================
        ...
        # ========================================
        # V411 ÇİFT ZAMAN DİLİMİ KİLİDİ (Hard MTF Filter)
        # ========================================
        ...
        try:
            ...
            ) # V411: Roketleri kaçırmamak için EMA50 ve EMA200 kilidi KALDIRILDI! Sadece EMA20 üstü olması yeterli.
        ...
        # ========================================
        # V411 MODÜLLERİ (Katman 2: Sadece Grafik Bazlı Olanlar)
        # ========================================
        ...
        # V411 YENİ: Rising stock tetikleyicisi
        ...
        # ========================================
        # FİNAL SONUÇ (V411 GENİŞLETİLMİŞ)
        # ========================================
        ...
        result = {
            ...
            # V411 Yeni alanlar
            "is_exhausted": is_exhausted,
            # Agresif Katman 2 & 3 için yeni alanlar
            "rvol_5_30": round(rvol_today_raw, 3), # İsim aynı kaldı ama içine BUGÜNÜN (Today) hacim patlaması atandı!
        ...
        logging.info(f"✅ {ticker}: V411 Analiz tamamlandı (Skor: {score:.2f}{exhaust_tag})")
        return result
...
# NY 14:00, Hafta içi çalışma, 168h Universe Cache, Faster I/O
# ============================================================
...
async def scan_top_stocks():
    ...
    - New York 14:00 zamanlamasıyla uyumlu çalışır.
    ...
    header = (
        f"🦅 <b>ATMACA SWING MASTER V411 – TOP 20 PROJEKSİYON</b>\n"
    ...
    if second_batch:
        report2_header = "🦅 <b>ATMACA SWING MASTER V411 – SON 10 DETAY RAPORU</b>\n"
    ...
    logging.info(f"✅ NY 14:00 Taraması başarıyla tamamlandı. ({scanned_count} hisse taranmış)")
    
from zoneinfo import ZoneInfo
from datetime import datetime, timedelta, timezone

def get_next_weekday_run_time_ny(target_hour=14, target_minute=0):
    """
...
async def run_scanner():
    ...
    await send_telegram_message(
        "🚀 <b>ATMACA Agresif V411 Başlatıldı!</b>\n"
        "⏱ Çalışma: Hafta içi her gün New York 14:00.\n"
        "🆕 <b>V411 Hyper-Momentum Kuralları:</b>\n"
        "  • 🥇 K1: Ani Hacim Radarı (Son 2 Gün RVOL > 1.2x)\n"
        "  • 🥈 K2: Taze Para & Kırılım (BUGÜN RVOL > 1.5x, Fiyat > EMA20, ADX > 18)\n"
        "  • 🥉 K3: Roket Sıralama → Saniyeler içinde en iyi 200 hisse"
    )
    ...
    while True:
        try:
            ...
            # Tarama başlat
            logging.info("▶ NY 14:00 taraması başlıyor...")
            await scan_top_stocks()
