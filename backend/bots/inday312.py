...
# --- AI API KEYS ---
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "")  # Key sadece Railway env'de olmalı, koda yazma!
...
Ana tarama fonksiyonu
    ...
    # ================================================
    # STAGE 3: AI FINAL SELECTION (GEMINI)
    # ================================================
    print(f"\n🤖 STAGE 3: AI FINAL SELECTION (GEMINIONLY)")
    print(f"   Analyzing {len(enriched_candidates)} validated candidates...\n")
    
    # Varsayılan olarak teknik adayları kabul et (Hata olursa bunlar gönderilir)
    final_results = enriched_candidates
    
    try:
        # AI Selection
        final_ai_selections = await ai_final_selection(
            enriched_candidates=enriched_candidates,
            gemini_client=gemini_client,
            top_n=40
        )
        
        # 1. Senaryo: AI Başarılı Oldu
        if final_ai_selections:
            final_results = final_ai_selections  # Sonuçları AI seçimi ile güncelle
            
            # AI Özetini Gönder
            try:
                ai_summary = f"🤖 <b>GEMINI 2.0 AI DOĞRULAMA</b>\n\n"
                ai_summary += f"📊 Analiz edilen: {len(enriched_candidates)}\n"
                ai_summary += f"✅ Seçilen: {len(final_ai_selections)}\n\n"
                
                ai_summary += "<b>EN İYİ SEÇİMLER (GEMINI 2.0):</b>\n"
                for idx, c in enumerate(final_ai_selections[:7], 1): # Top 7 özet
                    val = c.get('ai_validation', {})
                    ai_summary += (
                        f"{idx}. <b>{c.get('symbol')}</b> (Güven: {val.get('confidence', 0)}/10)\n"
                        f"   📝 {val.get('key_strength', '')}\n"
                        f"   🟢 Entry: {val.get('ai_entry', 'N/A')}\n"
                        f"   🛑 SL: {val.get('ai_sl', 0)} | 🎯 TP: {val.get('ai_tp', 0)}\n"
                        f"   ⏳ Vade: {val.get('hold_period', '2-5 Gün')}\n\n"
                    )
                
                #await send_telegram_message(ai_summary)
            except Exception as e:
                logging.error(f"Telegram summary error: {e}")

            # Sonra Detaylı Raporu Gönder
            try:
                ai_report = generate_ai_report(final_ai_selections)
                await send_telegram_message(ai_report)
                print("✅ AI Selection Report sent to Telegram")
            except Exception as e:
                logging.error(f"Telegram report error: {e}")
        
        else:
            print("⚠️ AI selection returned no candidates")
            # AI boş dönerse teknik listeyi gönder (Fallback Mesajı)
            fallback_msg = generate_telegram_report(enriched_candidates[:10])
            await send_telegram_message("⚠️ <b>AI Servisi Yanıt Vermedi</b>\nTeknik olarak seçilen hisseler gönderiliyor:\n\n" + fallback_msg)

    except Exception as e:
        logging.error(f"AI selection failed: {e}")
        print(f"⚠️ AI selection failed: {e}")
        
        # 2. Senaryo: AI Hata Verdi (Fallback)
        # Hata durumunda eldeki 'final_results' (enriched_candidates) kullanılır.
        final_results = enriched_candidates[:40] 
        
        # Hata mesajını ve manuel listeyi Telegram'a zorla gönder
        try:
            print("📨 Sending Fallback Report to Telegram...")
            fallback_report = generate_telegram_report(final_results, limit=10)
            error_header = f"⚠️ <b>AI SYSTEM ERROR (Fallback Mode)</b>\n<i>Hata: {str(e)[:50]}...</i>\n\n⬇️ <b>TECHNICAL SELECTION ONLY:</b>\n\n"
            await send_telegram_message(error_header + fallback_report)
            print("✅ Fallback Report sent to Telegram")
        except Exception as tg_err:
            logging.error(f"Could not send fallback telegram: {tg_err}")
...
def get_next_run_time_ny() -> datetime:
    """
    Calculate next scheduled run time (NY timezone)
    Schedule: ONLY 14:30 NY (Critical Session)
    """
