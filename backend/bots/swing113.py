...
ENABLE_TELEGRAM_NOTIFICATIONS = False  # swing113: Telegram kapalı — sadece JSON output
...
# ========================================
        ...
        # Eşik değeri 1.20'den 1.30'a çıkarıldı (2 günlük hareket daha belirgin/keskin olmalı)
        if rvol_micro < 1.30:
            layer2_pass = False
            layer2_reasons.append(f"Micro-RVOL(2g/20g)={rvol_micro:.2f} < 1.30")
...
async def scan_top_stocks():
    ...
    # ============================================================
    # 🥉 KATMAN 3: AĞIR VERİLER (Insider, Finansal, Info, Yasal Risk, Opsiyon) SADECE TOP 40 İÇİN
    # ============================================================
    logging.info("🏦 Katman 3 Başlıyor: Top 40 hisse için Ağır Veriler (Insider, Finansal, Yasal, Opsiyon) çekiliyor...")
    ...
    async def fetch_heavy_data_for_candidate(c: dict):
        ...
        try:
            ...
            # 🔮 V112 PREDICTIVE OPTIONS (Öncü Yön Tahmini - KARTAL V5.0)
            # Fiyat kopmadan önce akıllı paranın opsiyon zincirindeki yığılmasını tespit et
            opt_sentiment = await analyze_options_sentiment(ticker)
            # Eğer bullish sinyal varsa veya PCR çok yüksekse (uyarı için) listeye dahil et
            if opt_sentiment['bullish'] or opt_sentiment.get('pcr', 1.0) > 1.2:
                c["score"] += opt_sentiment['score']
                c["details"].extend(opt_sentiment['details'])
    ...
    # ============================================================
    # 💾 swing113 JSON OUTPUT — Fırsatlar için kaydediliyor
    # ============================================================
    try:
        ...
        now_ny_out = datetime.now(ZoneInfo("America/New_York"))
        timestamp_str = now_ny_out.strftime("%Y%m%d_%H%M")

        opportunities = []
        for rank, c in enumerate(top_candidates[:10], 1):
            ticker = c.get("ticker", "")
            price = float(c.get("current_price", 0))
            stop_loss = float(c.get("stop_loss", price * 0.95))
            tp2 = float(c.get("profit_target", price * 1.10))
            tp1 = price + (tp2 - price) * 0.40
            entry_low = round(price * 0.995, 2)
            entry_high = round(tp1, 2)
            ...
            opportunities.append({
                ...
                "reason": f"ATMACA V113 Score: {score}",
            })

        payload = {
            "run_timestamp": now_ny_out.strftime("%Y-%m-%d %H:%M:%S"),
            "run_time_ny": now_ny_out.strftime("%H:%M"),
            "bot_name": "swing113",
            "count": len(opportunities),
            "opportunities": opportunities,
        }

        latest_file = os.path.join(output_dir, "swing113_latest.json")
        archive_file = os.path.join(output_dir, f"swing113_{timestamp_str}.json")

        for fpath in [latest_file, archive_file]:
            with open(fpath, "w", encoding="utf-8") as jf:
                json.dump(payload, jf, indent=2, ensure_ascii=False, default=str)

        logging.info(f"📄 swing113 JSON kaydedildi: {latest_file} ({len(opportunities)} fırsat)")
    except Exception as e:
        logging.error(f"swing113 JSON kaydetme hatası: {e}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="swing113 — Fırsatlar Tarayıcı (Telegram'sız)")
    parser.add_argument("--one-shot", action="store_true", help="Bir kez çalıştır ve çık (APScheduler ile kullanım için)")
    ...
    try:
        ...
        if args.one_shot:
            logging.info("🚀 swing113 ONE-SHOT modunda çalıştırılıyor...")
            asyncio.run(scan_top_stocks())
        else:
            asyncio.run(run_scanner())

    except KeyboardInterrupt:
        print("\n🦅 swing113 manuel olarak durduruldu.")
    except Exception as e:
